...
# this machine. One script, five kinds:
#
...
cap_init "install.removeArtifact" \
    "Remove one installer-created artifact: binary | hostsEntries | mkcertCA | stack | images."
cap_spec_param "kind"         "what to remove: binary | hostsEntries | mkcertCA | stack | images"
...
function main() {
    ...
    case "$kind" in
        ...
        *) cap_fail 2 "unknown kind: ${kind} (supported: binary, hostsEntries, mkcertCA, stack, images)" ;;
    esac
}

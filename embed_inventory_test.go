...
var embedInventory = map[string]int{
	...
	"github.com/znasllc-io/memql/dsl":                    411, // all:<domain> x38 (+observability queries -- memql#4208; +shopify thin index -- memql#4137; +campaigns, +integrations -- memql#3323; +portalviews -- memql#3320; +install concepts+actions -- memql#3371; +campaigns builtins -- memql#3348; +campaigns automations -- memql#3461; +platform seeds -- the portal-is-site-1 seed, memql#3711; +library tools -- artifact labels, memql#4290; +platform builtins -- sitePublishFromArtifact, memql#4345); +161 for the GENERATED Shopify mirror and its hand-written neighbours -- 65 concepts, 65 fetch/list selections, 24 bulk documents and two namespace.pins from cmd/shopifyschema (so this number moves whenever allowlist.yaml does), plus the shopify overlay and the new `commerce` domain, less the six thin-index files they superseded (memql#4389); +1 for dsl/cluster/builtins.memql, the release-cut pair (epic memql#4434); +1 for prompts/uiAssistFill.tmpl, the uiAssist suggest domain (memql#4654); +1 for prompts/composeView.tmpl, the describe-a-view prompt (epic memql#4661) -- TWO epics each bumped this to 406 for a DIFFERENT template, so either side of that merge conflict would have been one short and the gate would have caught a file that IS embedded as one that is not; +4 for the `os` domain -- concepts/shapes/queries/mutations, the roaming desktop (epic memql#4746)

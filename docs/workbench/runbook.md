...
in [production.md](production.md) and is deferred until you start
...
| `MEMQL_WORKBENCH_REMOTE`   | unset (false)                      | When truthy AND a `ForwardRouter` is wired, the agent's dispatch delegates to a remote workbench node via NodeService.Stream. See [production.md](production.md). Leave unset for the MVP path. |

...
| Refresh-token reuse | **DETECTED since memql#4329, not merely refused.** Refresh tokens are one-time-use and rotate on every `/auth/refresh`; each rotation records the SHA-256 of the token it RETIRED on a `v1:identity:authActivity` row. A presented token matching neither the current hash nor an in-grace previous one is looked up against those retired hashes: a hit is a replay, and the whole session is revoked (`revokedReason=reuse_detected`), a `refresh_token_reuse_detected` signal lands on `v1:identity:auditEvent` with the presenting IP and User-Agent, the user is notified, and the caller gets 401 with `ErrTokenMismatch`. A miss is the ordinary stale cookie and is left alone -- revoking on an ambiguous signal would be a self-inflicted denial of service. Before this, a replay resolved to no session and became `session_refresh_blocked{session_not_found}`, indistinguishable from an expired cookie; `ErrTokenMismatch` was declared and never returned |
| Refresh-token reuse OLDER than the activity window | **NOT DETECTED, by construction.** Detection reaches back exactly as far as `MEMQL_IDENTITY_AUTH_ACTIVITY_RETENTION_DAYS` (default 30, memql#4330): past it the row recording the retired hash has been hard-deleted and the replay degrades to `session_not_found`. Accepted rather than closed, because the window is chosen to exceed both the 14-day idle timeout (`MEMQL_IDENTITY_SESSION_IDLE_DAYS`) and the 30-day refresh-token TTL (`MEMQL_IDENTITY_REFRESH_TOKEN_TTL_SECONDS`) -- a token older than the window is already dead on its own account, so there is nothing left for detection to add. **Lowering the retention below either of those TTLs opens a real gap**, and nothing warns about it; the bound in `component/identity/config.go` is [1, 365] and does not know the other two values |
| The activity log grows without bound | Daily Go-side hard delete on the identity node (`component/identity/authactivity`), counted by `memql_auth_activity_pruned_total`. Distinct from `v1:identity:auditEvent`, whose daily sweep only COUNTS -- MemQL has no `delete()` mutation, and an append-only audit log cannot be soft-deleted without changing what it means. A flat zero on that counter for more than a day, on a cluster that authenticates anyone, means the sweep is not running -- and the first thing to break is reuse detection above, silently |
...
`NodeService.Stream` is pinned to `class="node"` identity-issued JWTs (#105). The verifier on every cluster node rejects every other class -- user JWTs, PATs, and worker tokens are all unable to drive the mesh. Provisioning + rotation: see [`docs/public/operate/auth/node-jwt.md`](../../public/operate/auth/node-jwt.md).
...
The Go voice-agent (`integrations/voice/agent/`) authenticates via an identity-issued `class="voice_agent"` JWT (#109). The interceptor on `MemqlService.Stream` admits the class and pins the call to `VoiceAgent*` payload types -- a leaked credential can't drive other RPCs. Provisioning + rotation: see [`docs/public/operate/auth/voice-agent-jwt.md`](../../public/operate/auth/voice-agent-jwt.md).
...
**This was `MEMQL_MASTER_KEY` until memql#3519.** Sharing one value meant the secret-decryption key was also a cluster-owner bearer token over the network — while the envelope's sealing CLI wrote it into `~/.bashrc` **by default** at the file's existing (typically world-readable) mode, and ESO delivered it to staging/production pods. The old justification ("anyone who can read `/var/lib/memql` secrets from the host can produce it") reasoned about host filesystem access and did not transfer to either of those locations. The two are now separate secrets: the master key decrypts, the operator key authenticates.

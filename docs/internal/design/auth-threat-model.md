...
The Go voice-agent (`integrations/voice/agent/`) authenticates via an identity-issued `class="voice_agent"` JWT (#109). The interceptor on `MemqlService.Stream` admits the class and pins the call to `VoiceAgent*` payload types -- a leaked credential can't drive other RPCs.

...
**Scope:** MemQL engine mesh (6 node-types) + the downstream product carrier + the product SPA, on AKS `aks-<install>`, proven in staging ahead of a prod cutover.
...
and the DSN, so the cluster's secret state becomes

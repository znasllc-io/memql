...
- Voice: the per-response `instructions` on `response.create`
  (the #432 design, `docs/internal/design/voice-432-conductor-response-gate.md:169-204`).
...
- **Voice runtime:** the gpt-realtime websocket session
  (`integrations/openai/realtime.go` per `docs/internal/design/voice-453-gpt-realtime-go.md`),
...
the same websocket
(`docs/internal/design/voice-453-gpt-realtime-go.md:249-260` shows the audio variant;
...
- **WHAT CONTEXT -- grounding.** RAG retrieval over the agent's domains, keyed
  ...
  the #432 design at `docs/internal/design/voice-432-conductor-response-gate.md:201-204`).
...
2. **Proto bump scope (stage 2).** Extending `VoiceAgentSessionAck`
   ...
   `docs/internal/design/voice-453-gpt-realtime-go.md:436-441`).

...
**1.3 Vector classification cache primitive.** New piece of infrastructure: a cache layer that takes `(input_text, classification_namespace) → ClassificationResult` using vector similarity over `node_vectors`. Configurable similarity threshold per namespace. Reuses existing embedding provider infrastructure. Falls through to fresh LLM call on miss, stores the result back. Becomes a single building block every classification call uses.

...
- **3.5 (#1902):** moves the **avatardirect + chat + dailyspace** Go integrations to the pack (blank-imported from the carrier anchor). **Correction: `avatarvendor` STAYS core** — it's a shared vendor-REST primitive the core voice-agent imports (`integrations/voice/agent/*`); the pack's avatardirect imports it cross-repo (pack→core). Knowledge integration **stays core**. The product builtin *declarations* (in core `dsl/cognition`+`dsl/common`) are dead-but-loadable in engine-only (loader skips `integration.*` executor validation) → spun out to **#1976** (coupled to 3.3). Merge order is **core-first** (RegisterPlugin panics on duplicate name).
- **3.6 (#1903):** investigation found library only *weakly* space-coupled — `spaceId` is one optional facet field woven through concepts/mutations/logic/shapes/Go, referencing `v1:cognition:space`. The only cleanly-separable construct is the per-space query `queryLibraryArtifactsBySpace` → **moved to the pack** (`dsl/copresent`); the artifact concept + facet field stay core (the provenance/version spine). The pervasive facet-field decoupling is **entangled with the space concept → deferred to 3.3 (#1900)** (owner decision 2026-06-22). All other former "split-case" concepts (forge, actions, calendar/notes/todos, liveknowledge, unmetCapability/guardrailHealth) are **confirmed CORE** — no move.

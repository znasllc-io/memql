...
| Layer | Change | Where |
|---|---|---|
| The plan | Composite index `(concept, id, "createdAt" DESC)` on `MemoryNodes`; the same query reads 379 rows instead of a million. Built per chunk (`transaction_per_chunk`) on a hypertable and plainly on a plain table, and never trusted by name: the migration inspects the catalog and accepts the index only when it is valid on the table and carried by every uncompressed chunk (or an operator's equivalent index already is), repairs an interrupted or hand-damaged one, and re-inspects after building. A build cut off by the driver's 10 s read deadline is waited for, not dropped | Go migrations `20260913000000_memory_nodes_concept_id_created_at_idx` (PR #5314, made Go by memql#5252) and `20260914000000_memory_nodes_concept_index_verified`, which re-runs the check on clusters that applied the earlier SQL form |

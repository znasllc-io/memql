...
`docs/voice/432-conductor-response-gate.md`) that **memQL's conductor decides
...
`docs/voice/eou-tuning.md` -- the Go client speaks Deepgram raw, so it **does**
...
1. **End-of-utterance (turn boundary)** -- "the human is done; commit the
   ...
   **client-side watchdog**. Deepgram's endpointing is *signal-aware* (it knows
   ...
   `docs/voice/eou-tuning.md` section "Why not just use VAD-based turn detection"

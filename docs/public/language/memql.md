...
>
> Two cases fail the *other* way and are worth knowing separately: **`[]any` and `map[string]any` constrain their elements to `string`**. `any` means "unconstrained" in property position and "must be a string" one level in, so the obvious freeform-metadata-bag declaration rejects `{"count": 3}` at runtime. Everything else in this note accepts too much; those two accept too little.
>

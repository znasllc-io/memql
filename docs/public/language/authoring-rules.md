...
2. ship a `memqlmigrate --rewrite=<epic>` mode that mechanically migrates
   ...
   `scripts/migrations/event_payload_args/`, and
   `memqlmigrate --rewrite=expressions` for the edition-2026 expression
   grammar);
...
for every pair. Three results differ from before:

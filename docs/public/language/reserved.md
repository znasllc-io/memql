...
| `@internal` | RETIRED at construct level under the 2026.08 epoch (#2620 ruling / #2708) -- it only hid a callable construct from discovery surfaces; the load gate rejects it with a migration hint. Field-level `@internal` on concept properties remains live, and is enforced (rejected from caller args + excluded from a shape's default projection). Its neighbours are not uniform: `@pii` is enforced (`@scrubPii` + the memql#2883 projection gate), while `@secret` is only PARTIALLY enforced (memql#3036) -- it redacts a rejected value in exactly one validation surface, the **function-args validator**, and matches by argument NAME rather than by write target (a mutation writing `apiKey: args.credential` leaves `credential` unredacted). The uncovered surfaces named here are those verified uncovered, not a closed set -- treat one absent from the list as unclassified rather than covered. It does NOT redact from **query results** (an authorization decision deferred under memql#2803), IS also redacted by the **automation args binder** (memql#3111) -- a second validator over event payloads, which used to quote the value in full and write it to a WARN log; it recovers the concept from the event topic at bind time, matching by field name -- but NOT from the **tool-args validator** (`validateToolArgs`), compiled from the same args schema, auto-registered for every enabled query and mutation, running BEFORE the covered validator on the agent path and WARN-logging the entire args map (memql#3117) -- so a row value CAN still reach a **structured log** by THAT path -- and NOT from **concept payload validation**, where `@minimum` / `@maximum` / `@format` are enforced by JSON schema, which interpolates the value. Length is never redacted anywhere (`value too long` reports a rune count for a secret field too). Treat `@secret` as narrowing one diagnostic surface, not as a secrecy guarantee. |

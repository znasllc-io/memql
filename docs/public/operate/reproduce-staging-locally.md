...
|---|---|---|---|---|---|
| 1 | **identity replica count** -- RESOLVED (#1304) | **2** (hostname-derived node id, NO host port) | 2 (`fieldRef` id) | Now at full replica parity. Earlier kept at 1 because the `8081:8081` host-port binds only one replica and the static `MEMQL_NODE_ID` collides across replicas (#1042). #1304 closes the gap: drop the host port and reach identity ONLY through the nginx front door (which path-routes `/auth/*` except the SPA-owned `/auth/callback`+`/auth/oauth`, plus `/oauth/*`, `/.well-known/*`, `/setup`, `/admin` to `identity:8081`, DNS-round-robined across both replicas); drop the static id (hostname-derived, #1213); move `IDENTITY_BASE_URL` + the issuer claim onto the `:8085` front door (`IDENTITY_VERIFIER_BASE_URL` still uses the in-cluster `identity:8081` service name for JWKS fetch). | **RESOLVED #1304** -- verify login + JWKS on a live `make dev-cluster-refresh` |

...
- Messages (exact prefixes, the corpus and tests pin them):
  - missing: `domain "znas" declares no language line: add znas/memql.toml containing` + the two rendered lines + `(or run: memqlmigrate --rewrite=language-line -w <dir>, where <dir> is the directory that holds znas/) [language_line_missing]`

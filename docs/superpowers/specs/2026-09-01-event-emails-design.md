...
- **Scope:** `dsl/campaigns/` (the `emailRule` concept + its queries,
  ...
  a deterministic construct generator in `component/campaigns`, a
...
| E7 | Edit is retire-then-arm | Editing an active rule generates a NEW bundle and retires the old one, in that order at the row level and the reverse order on the wire: `campaignRetireEmailRule` first, then generate and activate. A rule is therefore never armed twice, which is the only failure here that produces DUPLICATE MAIL rather than none. `constructName` derives deterministically from the rule id so a regeneration replaces rather than accumulates |

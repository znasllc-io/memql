...
/* THE LAYOUT AND ROLE RULES ARE GONE (epic memql#5009, memql#5020).
   They dressed planLayout's slots and entryRole's three sizes, and both
   modules were removed with the portal that was their only consumer. The
   rules had to go with them: styles.test.ts fails the build on a class that
   has a rule and no renderer, which is exactly what these had become -- and
   that gate is what caught them, rather than anything a compiler could see.

   NOTE FOR ANYONE ADDING A RULE HERE: this whole sheet is a template
   literal, so a BACKTICK inside a comment ends the string. The errors it
   produces point at the line after the comment and say nothing about
   quoting. */
`;

...
`generated_logics.go`, `generated_concepts.go`, and
`generated_builtins.go` are produced by `scripts/sdk-gen` from the DSL
tree (all five carry the `Code generated by scripts/sdk-gen. DO NOT
EDIT.` header). Don't hand-edit. CI gate: `make sdk-gen-check`
regenerates and fails on drift. After any DSL change to a construct's
args / signature, run `make sdk-gen` locally and commit the result.
...
sdk/go/
  client/                  Connection, Dispatcher, QueryClient, SubscriptionManager
    ...
    generated_concepts.go Generated typed concept methods (DO NOT EDIT)
    generated_builtins.go Generated typed builtin methods (DO NOT EDIT)
    concept_browser.go    Admin-surface BrowseConcept / GetRowByConceptAndId
    support.go            Result type + executeNamed + renderMemQLValue
  authoring/               Bundle validate / session-define / promote SDK surface (memql#2128)

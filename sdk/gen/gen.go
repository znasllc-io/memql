...
func Generate(opts Options) (*Result, error) {
	...
	files := Emit(constructs, opts.GoOut, opts.TSOut)
...
func Emit(constructs []Construct, goOut, tsOut string) []OutputFile {
	...
	if strings.TrimSpace(tsOut) != "" {
		out = append(out,
			OutputFile{tsOut, "generated_queries.ts", emitTSMethods(byKind["query"], "query")},
			OutputFile{tsOut, "generated_mutations.ts", emitTSMethods(byKind["mutation"], "mutation")},
			OutputFile{tsOut, "generated_logics.ts", emitTSMethods(byKind["logic"], "logic")},
		)
	}
	return out
}

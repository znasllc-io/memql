...
test("executeNamed -- forwards opts.cursor onto ExecuteQueryMsg.cursor", async () => {
  ...
  const promise = qc.executeNamed("querySpaceUtterances", 'querySpaceUtterances({"spaceId":"s1"})', {
...
test("executeRaw -- forwards opts.cursor onto the wire", async () => {
  ...
  const promise = qc.executeRaw('paginate(sort(querySpaceUtterances({"spaceId":"s1"}), "createdAt", "desc"), 50)', {
    cursor: "raw-cursor",
  });
  mock.reply({
    queryResult: { requestId: "r1", result: { data: [] } },

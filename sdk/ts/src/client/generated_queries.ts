...
The composite-tier term is `campaigns`' own conjunct, repeated here rather than referenced: this is the same read that query makes, narrowed by the tie field. `sitesForAccount` above dropped its copy under memql#5303 (D4) because v1:platform:site declares the account argument; campaigns keep theirs, because a rollup that dropped the tier predicate would be the shortest path in the product to reading every operator's campaigns.
...
/** The caller's live file at one content digest -- the dedup read behind content-addressed storage (epic memql#5396, design D5).
...
// Bound concept: v1:library:file (machine-readable: BoundConcepts["libraryFileBySha256"] in generated_concepts.ts).
export interface LibraryFileBySha256Args {
  sha256: string;
}

export function buildLibraryFileBySha256(args: LibraryFileBySha256Args): string {
  const parts: string[] = [];
  parts.push("sha256: " + renderMemQLValue(args.sha256));
  return "query libraryFileBySha256(" + parts.join(", ") + ")";
}

declare module "./query.js" {
  interface QueryClient {
    libraryFileBySha256(args: LibraryFileBySha256Args, opts?: QueryCallOptions): Promise<Result>;
  }
}

QueryClient.prototype.libraryFileBySha256 = function (this: QueryClient, args: LibraryFileBySha256Args = {} as LibraryFileBySha256Args, opts?: QueryCallOptions): Promise<Result> {
  return this.executeNamed("libraryFileBySha256", buildLibraryFileBySha256(args), opts);
};
...
export function buildModuleReadinessAll(args: ModuleReadinessAllArgs): string {
  void args;
...
declare module "./query.js" {
  interface QueryClient {
    moduleReadinessAll(args?: ModuleReadinessAllArgs, opts?: QueryCallOptions): Promise<Result>;
  }
}
...
CLUSTER-OWNER TIER BY ITS ACTOR rather than by a spec: the concept's composite tier already admits a cluster owner to every row, and the sweep runs under the maintenance actor, which IS one. So the filter says only what it is looking for -- a run at a non-terminal status -- and the tier decides who may see it (memql#5303, D4: no caller term, as every read in this section). The same query read by an ordinary caller answers their own stranded runs and those of the packages they are tied to, which is correct and is not a second code path.
...
v1:router:call declares no @rowAuthz tier, so this gate is the whole of the authorization. Declaring a tier on the concept would narrow every existing read of the cost ledger, and none of the tiers the model can express is true of this row -- memql#5162 records why.
...
/** Get a user by id -- FULL row, for a caller holding `read` on `principal`.

...
export function buildAutomationGraph(args: AutomationGraphArgs): string {
  void args;
...
declare module "./query.js" {
  interface QueryClient {
    automationGraph(args?: AutomationGraphArgs, opts?: QueryCallOptions): Promise<Result>;
  }
}

QueryClient.prototype.automationGraph = function (this: QueryClient, args: AutomationGraphArgs = {} as AutomationGraphArgs, opts?: QueryCallOptions): Promise<Result> {
  return this.executeNamed("automationGraph", buildAutomationGraph(args), opts);
};

/** The latest 50 runs the loop protection stopped, newest first, one v1:platform:automationLoopStop row each: the chain of automation names and run ids that led to the stop, the bound it passed, and when. Read by the engine under its own cluster actor and projected without payloads -- no input, variables or triggering event reaches the reply. */
export interface AutomationLoopStopsArgs {
}

export function buildAutomationLoopStops(args: AutomationLoopStopsArgs): string {
  void args;
...
declare module "./query.js" {
  interface QueryClient {
    automationLoopStops(args?: AutomationLoopStopsArgs, opts?: QueryCallOptions): Promise<Result>;
  }
}
...
/** Start a Materializer work run using the known materializeFile template. The composition and immutable source snapshot are saved before dispatch. Returns {compositionId, goalId, runId}; progress and the eventual outputFileId arrive on the composition row. */
...
/** Ask for a specialist to be trained, as a decision a person makes rather than a thing an agent does. Raises a skillMint approval on one of the caller's runs; approving it opens the trainSpecialist work -- the SAME template the refresh cadence opens, so approval and cadence cannot drift into two trainers.
...
export interface RequestSpecialistTrainingArgs {
...
export function buildRequestSpecialistTraining(args: RequestSpecialistTrainingArgs): string {
  const parts: string[] = [];
  parts.push("runId: " + renderMemQLValue(args.runId));
  parts.push("specialistId: " + renderMemQLValue(args.specialistId));
  if (args.topic !== undefined) parts.push("topic: " + renderMemQLValue(args.topic));
  if (args.mode !== undefined) parts.push("mode: " + renderMemQLValue(args.mode));
  return "builtin requestSpecialistTraining(" + parts.join(", ") + ")";
}

declare module "./query.js" {
  interface QueryClient {
    requestSpecialistTraining(args: RequestSpecialistTrainingArgs, opts?: QueryCallOptions): Promise<Result>;
  }
}
...
/** Record that an order reached the mirror under a store a preview is open against -- the FOURTH and only passive observation (issue memql#5547).
...
export interface SitePreviewNoteOrderArgs {
...
export function buildSitePreviewNoteOrder(args: SitePreviewNoteOrderArgs): string {
  const parts: string[] = [];
  parts.push("storeId: " + renderMemQLValue(args.storeId));
  if (args.orderId !== undefined) parts.push("orderId: " + renderMemQLValue(args.orderId));
  return "builtin sitePreviewNoteOrder(" + parts.join(", ") + ")";
}

declare module "./query.js" {
  interface QueryClient {
    sitePreviewNoteOrder(args: SitePreviewNoteOrderArgs, opts?: QueryCallOptions): Promise<Result>;
  }
}

QueryClient.prototype.sitePreviewNoteOrder = function (this: QueryClient, args: SitePreviewNoteOrderArgs = {} as SitePreviewNoteOrderArgs, opts?: QueryCallOptions): Promise<Result> {
  return this.executeNamed("sitePreviewNoteOrder", buildSitePreviewNoteOrder(args), opts);
};

/** Open a short-lived preview of a deployable's candidate version, on the deployable's OWN origin (issue memql#5545).
...
export interface SitePreviewOpenArgs {
  /** The deployable to preview. */
  siteId: string;
}

export function buildSitePreviewOpen(args: SitePreviewOpenArgs): string {
  const parts: string[] = [];
  parts.push("siteId: " + renderMemQLValue(args.siteId));
  return "builtin sitePreviewOpen(" + parts.join(", ") + ")";
}

declare module "./query.js" {
  interface QueryClient {
    sitePreviewOpen(args: SitePreviewOpenArgs, opts?: QueryCallOptions): Promise<Result>;
  }
}

QueryClient.prototype.sitePreviewOpen = function (this: QueryClient, args: SitePreviewOpenArgs = {} as SitePreviewOpenArgs, opts?: QueryCallOptions): Promise<Result> {
  return this.executeNamed("sitePreviewOpen", buildSitePreviewOpen(args), opts);
};

/** Run the three things the engine can ACTIVELY observe of a preview and record each on the grant (issue memql#5547): the development store answers a catalog read, a cart accepts a line, a checkoutUrl comes back.
...
export interface SitePreviewProbeArgs {
  /** The preview to record the observations on. */
  grantId: string;
}

export function buildSitePreviewProbe(args: SitePreviewProbeArgs): string {
  const parts: string[] = [];
  parts.push("grantId: " + renderMemQLValue(args.grantId));
  return "builtin sitePreviewProbe(" + parts.join(", ") + ")";
}

declare module "./query.js" {
  interface QueryClient {
    sitePreviewProbe(args: SitePreviewProbeArgs, opts?: QueryCallOptions): Promise<Result>;
  }
}

QueryClient.prototype.sitePreviewProbe = function (this: QueryClient, args: SitePreviewProbeArgs = {} as SitePreviewProbeArgs, opts?: QueryCallOptions): Promise<Result> {
  return this.executeNamed("sitePreviewProbe", buildSitePreviewProbe(args), opts);
};

/** What is legal for a deployable right now, in the guard's own words (issue memql#5546).
...
export interface SitePreviewReadinessArgs {
  /** The deployable to ask about. */
  siteId: string;
}

export function buildSitePreviewReadiness(args: SitePreviewReadinessArgs): string {
  const parts: string[] = [];
  parts.push("siteId: " + renderMemQLValue(args.siteId));
  return "builtin sitePreviewReadiness(" + parts.join(", ") + ")";
}

declare module "./query.js" {
  interface QueryClient {
    sitePreviewReadiness(args: SitePreviewReadinessArgs, opts?: QueryCallOptions): Promise<Result>;
  }
}

QueryClient.prototype.sitePreviewReadiness = function (this: QueryClient, args: SitePreviewReadinessArgs = {} as SitePreviewReadinessArgs, opts?: QueryCallOptions): Promise<Result> {
  return this.executeNamed("sitePreviewReadiness", buildSitePreviewReadiness(args), opts);
};
...
/** Ask one store's Storefront API the three questions a storefront preview can answer (epic memql#5531, issue memql#5547): does the catalog read, does a cart accept a line, does a checkoutUrl come back.
...
export interface StorefrontProbeArgs {
  /** The store to ask. The only configured store when omitted. */
  storeId?: string;
}

export function buildStorefrontProbe(args: StorefrontProbeArgs): string {
  const parts: string[] = [];
  if (args.storeId !== undefined) parts.push("storeId: " + renderMemQLValue(args.storeId));
  return "builtin storefrontProbe(" + parts.join(", ") + ")";
}

declare module "./query.js" {
  interface QueryClient {
    storefrontProbe(args: StorefrontProbeArgs, opts?: QueryCallOptions): Promise<Result>;
  }
}

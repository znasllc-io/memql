...
//
// A plugin built past v0.18.0 sent `AuthoringValidateBundleMsg.origin`. That
...
//
// Nothing anywhere said the cluster was simply older than the plugin.
//
...
// an ingress timing out, or a token expiring mid-stream. The plugin cannot
...
export interface SkewHintInputs {
   ...
   * hinting at version skew when the plugin itself closed the socket would be
   * nonsense.
   */
  reason?: string;
  /** The cluster's recorded version (memql#3990). */
  recorded: string | undefined;
  /** The release this plugin is built against. Defaults to the shipped pin. */
  pluginTag?: string;
}
 ...
 *   - and it is genuinely BEHIND this plugin (a cluster at or ahead of the
 *     plugin cannot be refusing a field the plugin learned later, and telling
...
export function withVersionSkewHint(inputs: SkewHintInputs): string {
  ...
  const pluginTag = inputs.pluginTag ?? DEFAULT_STACK_TAG;
  if (compareVersions(inputs.recorded, pluginTag) !== "behind") return inputs.message;

  const recorded = (inputs.recorded ?? "").trim();
  return (
    `${inputs.message} ` +
    `This cluster records ${recorded} and the plugin is built against ${pluginTag}, ` +
    `so the session may have been closed by a field this plugin sends that ${recorded} does not accept ` +

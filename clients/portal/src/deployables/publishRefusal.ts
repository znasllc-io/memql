...
// ===========================================================================
//
// Every refusal path in integrations/library/site_publish.go names itself with
// a STABLE reason -- `artifact_not_a_zip`, `bundle_missing_index` -- and that
// is deliberate on both ends: the audit row records the reason so it can be
// searched, and this page renders it so the person is not shown a token. The
// server's `Detail` half is prose that may be edited at any time and is
// written for an operator reading a log; the REASON is the contract.
//
...
// error string, sdk/ts's executeNamed prefixes it with the call name, and the
// capability's own Error() renders `sitePublishFromArtifact refused: <reason>
// -- <detail>`. Rather than parse that exact composition -- which would break
// the first time a layer added a prefix -- this scans the whole message for
// any known reason as a WHOLE WORD. The reasons are underscored identifiers
// that appear in no English sentence, so a scan cannot collide with prose.
//
...
export const PUBLISH_REFUSALS: Readonly<Record<string, string>> = {
  missing_argument:
    "The deploy call was incomplete -- it named no deployable or no artifact. Pick a bundle and try again.",

  // The two "not found" reasons cover being refused as much as being absent,
  // and saying so is the honest form: both reads are owner-scoped, so
  // somebody else's row resolves to zero rows rather than to a permission
  // error. A message that only said "does not exist" would send a person
  ...
  artifact_archived:
    "That bundle is archived. Restore it in the Library, or pick another one.",
  artifact_not_a_file:
    "That Library entry has no file behind it -- a note, a to-do or a memory is a record, not bytes. Pick an uploaded zip.",
  file_not_found: "The file behind that Library entry is gone, so there is nothing to deploy.",
  file_archived: "The file behind that Library entry is archived, so there is nothing to deploy.",
  artifact_not_a_zip:
  ...
  bundle_unreadable: "The bundle's bytes could not be read from storage. Try again; if it keeps failing, the upload may not have completed.",
  bundle_not_a_zip:
  ...
  bundle_file_too_large: "One file inside the bundle is over 25 MB. Move anything that big out of the bundle.",
...
// publishRefusalReason returns the stable reason carried anywhere in a message,
// or "" when there is none.
export function publishRefusalReason(message: string): string {
  for (const reason of Object.keys(PUBLISH_REFUSALS)) {
    // Whole-word match: `bundle_too_large` must not match inside
...
// describePublishFailure is what the page renders. A known refusal becomes its
// sentence; anything else keeps its own message.

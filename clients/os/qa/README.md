...
`source-chooser` supplies multiple GitHub identities, personal/org bindings and repositories. Open Add a deployable, choose A repository, select a Source, then its repository. MemQL account ownership appears afterward. Add source saves only an identity/installation binding. Check per-source removal and cancellation, account cue placement, repository/ref clearing on Source change, keyboard navigation, desktop/narrow layouts and both themes. `source-management` opens the same Sources manager directly; `source-empty` starts without bindings.

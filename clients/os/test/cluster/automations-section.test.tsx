...
describe("Automations",()=>{
 ...
 it("shows empty and disconnected states honestly",async()=>{const view=render(<AutomationsSection/>);expect(screen.getAllByText("Waiting for a cluster connection.")).toHaveLength(2);h.connection=connection([]);view.rerender(<AutomationsSection/>);expect(await screen.findByText("This cluster loaded no automations.")).not.toBeNull();expect(screen.getByText(/No loop has been stopped/)).not.toBeNull();});

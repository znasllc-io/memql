...
export function LoopStops({reading}:{reading:Reading<LoopStop[]>}) {
  return <section className="os-automation-stops" aria-label="Loops stopped"><Subhead meta={reading.state === "read" && !reading.error ? reading.value?.length : undefined}>Loops stopped</Subhead>{reading.state==="failed"?<Notice tone="error" sentence="The cluster did not answer the stopped chains." detail={reading.error}/>:reading.value===null?<Caption>{reading.state==="unread"?"Waiting for a cluster connection.":"Reading stopped chains."}</Caption>:reading.value.length===0?<Caption>No loop has been stopped in the runs this cluster still keeps.</Caption>:<><Caption>Read at {reading.at?.toLocaleTimeString()}. Latest retained stops.</Caption><RecordList as="ol" label="Stopped automation chains">{reading.value.map((row,i)=><Stop key={row.runId || row.id || i} row={row}/>)}</RecordList></>}</section>;
}

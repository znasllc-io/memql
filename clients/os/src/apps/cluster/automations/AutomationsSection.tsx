...
export function AutomationsSection() {
  ...
  return <div ref={root} className={`os-automations ${row?"has-selection":""}`} onKeyDown={e=>{if(e.key==="Escape"){e.stopPropagation();clear();}}}>
    <Head title="Automations" meta={graph.state === "read" && !graph.error ? graph.value?.length : undefined}><Button tone="quiet" busy={graph.state==="reading"||stops.state==="reading"} onClick={reread}>Read again</Button></Head>

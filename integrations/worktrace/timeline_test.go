...
func TestAssembleTrace_OrdersByCreatedAt(t *testing.T) {
	...
	events := []TraceEvent{
		{At: at(base, 5), Kind: EventKindStep, NodeID: "s1", StepID: "s1", Status: StepStatusDone, Mutation: "updateWorkStep"},
		{At: at(base, 0), Kind: EventKindRun, NodeID: "p1", Status: RunStatusRunning, Mutation: "createWorkRun", Content: "the goal"},
		...
		{At: at(base, 2), Kind: EventKindStep, NodeID: "s1", StepID: "s1", Status: StepStatusRunning, Mutation: "createWorkStep"},
		{At: at(base, 1), Kind: EventKindStep, NodeID: "s1", StepID: "s1", Status: StepStatusReady, Mutation: "createWorkStep"},
		{At: at(base, 6), Kind: EventKindRun, NodeID: "p1", Status: RunStatusSucceeded, Mutation: "updateWorkRun", Content: "the goal"},
	}

	tr := AssembleTrace("p1", events)

	if tr.RunID != "p1" {
		t.Fatalf("RunID = %q, want p1", tr.RunID)
	}
	if tr.Goal != "the goal" {
		t.Fatalf("Goal = %q, want 'the goal'", tr.Goal)
	}
	if tr.FinalStatus != RunStatusSucceeded {
...
func TestComputeMetrics_FromTrace(t *testing.T) {
	base := time.Date(2026, 6, 1, 12, 0, 0, 0, time.UTC)
	events := []TraceEvent{
		{At: at(base, 0), Kind: EventKindRun, NodeID: "p1", Status: RunStatusRunning, Content: "g"},
		...
		{At: at(base, 10), Kind: EventKindRun, NodeID: "p1", Status: RunStatusFailed, Content: "g"},
	}
	tr := AssembleTrace("p1", events)
	m := ComputeMetrics(tr)
...
func TestComputeMetrics_ToolResultCountsAsOneCall(t *testing.T) {
	base := time.Date(2026, 6, 1, 12, 0, 0, 0, time.UTC)
	events := []TraceEvent{
		{At: at(base, 0), Kind: EventKindRun, NodeID: "p1", Status: RunStatusSucceeded, Content: "g"},
...
// sequencesEqual is the test-local helper the harness suite carried; kept
// here so the ported cases assert the same thing they did.

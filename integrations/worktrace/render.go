...
// render.go renders a reconstructed Trace as a plain-text timeline. It
// lives beside the assembler (not in the cockpit) so a `work trace <runId>`
// command is a thin caller: fetch the trace, hand it to Render, print. Pure
// formatting, no DB / TUI deps, so it is unit-testable and reusable by any
// surface (CLI, logs, a BFF endpoint).
...
// Render returns a human-readable rendering of the run timeline: a header
// (run id / goal / final status), then each event in createdAt order with
...
func (t Trace) Render() string {
	...
	if len(t.Events) == 0 {
		b.WriteString("\n  (no timeline events -- run not found or no activity)\n")
		return b.String()
	}
...
func renderEventLine(ev TraceEvent) string {
	switch ev.Kind {
	case EventKindRun:
		return fmt.Sprintf("[run]   %-8s %s%s", ev.Status, mutationLabel(ev.Mutation), actorSuffix(ev.Actor))
	case EventKindStep:
		label := ev.StepID
		if ev.Content != "" {

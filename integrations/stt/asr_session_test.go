...
// mockASRStream implements audio.ASRStream for tests, letting us control
...
func newMockASRStream(buffer int) *mockASRStream {
	return &mockASRStream{results: make(chan audio.ASRResult, buffer)}
}
...
func (m *mockASRStream) Results() <-chan audio.ASRResult { return m.results }
...
// newTestSession wires a asrSession directly to a mock upstream
// stream, mirroring what a provider's StartStream builds at runtime.
func newTestSession(stream audio.ASRStream) *asrSession {
	return newASRSession(stream, "openai-realtime", slog.New(slog.DiscardHandler))
}

func TestSession_ForwardsInterimAndFinal(t *testing.T) {
	mock := newMockASRStream(4)
	mock.results <- audio.ASRResult{Text: "hey sof", IsFinal: false, Confidence: 0.6}
	mock.results <- audio.ASRResult{Text: "hey sofia", IsFinal: true, Confidence: 0.95}
	...
	// The ASR contract carries float64 confidence; we downcast to float32.
...
func TestSession_Finalize_AccumulatesMultipleFinals(t *testing.T) {
	mock := newMockASRStream(4)
	mock.results <- audio.ASRResult{Text: "first segment", IsFinal: true, Confidence: 0.9}
	mock.results <- audio.ASRResult{Text: "second segment final", IsFinal: true, Confidence: 0.92}
...
func TestPolyphonSession_AccumulatesAcrossUtterances(t *testing.T) {
	...
	mock.results <- audio.ASRResult{Text: "hey", IsFinal: false}
	mock.results <- audio.ASRResult{Text: "hey there", IsFinal: false}
	mock.results <- audio.ASRResult{Text: "Hey, there.", IsFinal: true, Confidence: 0.95}

	// Utterance 2: two interim deltas + final.
	mock.results <- audio.ASRResult{Text: "how", IsFinal: false}
	mock.results <- audio.ASRResult{Text: "how are you", IsFinal: false}
	mock.results <- audio.ASRResult{Text: "How are you?", IsFinal: true, Confidence: 0.96}
...
func TestSession_Finalize_EmptyWhenNoFinalArrived(t *testing.T) {
	mock := newMockASRStream(4)
	mock.results <- audio.ASRResult{Text: "interim only", IsFinal: false, Confidence: 0.5}

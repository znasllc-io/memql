...
// asrSession adapts an audio.ASRStream to stt.StreamingSession.
// It is shared by every STT provider that wraps a audio.ASRProvider --
...
// newASRSession wires a audio.ASRStream into a stt.StreamingSession
// and starts the forwarder goroutine. The provider name flows into the final
// transcription payload and the drop-log line.
func newASRSession(stream audio.ASRStream, provider string, logger *slog.Logger) *asrSession {
	if logger == nil {
...
func (s *asrSession) Receive() <-chan TranscriptionResult {
	return s.results
}
...
func (s *asrSession) Finalize(ctx context.Context) (*FinalTranscription, error) {
	s.mu.Lock()
	if s.closed {
		s.mu.Unlock()
	...
	if err := s.stream.Close(); err != nil {
		s.logger.Warn("asr: close stream", "provider", s.provider, "error", err)
	}
	...
	case <-time.After(3 * time.Second):
		s.logger.Warn("asr: forwardResults did not drain within 3s",
			"provider", s.provider)
	}
...
func (s *asrSession) finalResult() *FinalTranscription {
	s.mu.Lock()
	text := s.sessionText
	s.mu.Unlock()
...
// forwardResults reads ASRResult from the upstream stream and translates each
...
func (s *asrSession) forwardResults(in <-chan audio.ASRResult) {
	...
	for r := range in {
		...
		default:
			s.logger.Debug("asr: results channel full, dropping interim",
				"provider", s.provider)
		}
	}
}

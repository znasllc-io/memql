...
func (l *PlannerAgentLoop) dispatchDecision(ctx context.Context, planId string, d plannerDecision, iter int, conv *convTracker) error {
	...
	case "spawnTrainingPlan":
		...
		// Approved (metrics.specialistApproved / tokenCapDisabled): mint
		// the kind=trainSpecialist child Plan the TrainSpecialistDispatcher
		// (#644) claims + runs, then mark this Plan succeeded. memql#852
		// Gap 2.
		return l.dispatchApprovedTrainingPlan(ctx, planId, &d)
	case "retry":
...
type plannerDecision struct {
	...
	// Action == "spawnTrainingPlan"; the handler in
	// agent_loop_spawn_training.go mints a kind=trainSpecialist child
	// Plan from these once the gate has approved the request. The
	// target domain is resolved server-side (the new specialist's
	// attached domain), so the model never supplies a domainId.

...
type fakeEngine struct {
	...
	participants map[string][]map[string]any // partitionId -> active participant rows
	presence     map[string][]map[string]any // partitionId -> presence rows
...
func (f *fakeEngine) Execute(_ context.Context, query string) (*memql.ExecuteResult, error) {
	...
	case "createDailySpace":
		return rowsResult(nil), nil
	case "activeDailySpacesForUser":
		return rowsResult(f.dailySpaces[str("userId")]), nil
	case "spaceParticipants":
		partitionId := str("partitionId")
		status := str("status")
		var out []map[string]any
		for _, p := range f.participants[partitionId] {
	...
	case "leaveSpace":
		...
		partitionId, _ := payload["partitionId"].(string)
		for _, p := range f.participants[partitionId] {
			if p["id"] == pid {
				p["status"] = "left"
			}
		...
		return rowsResult(nil), nil
	case "spaceParticipantPresence":
		return rowsResult(f.presence[str("partitionId")]), nil
	case "updateParticipantPresence":
		f.presenceWrites = append(f.presenceWrites, args)
		partitionId := str("partitionId")
		presenceId := str("presenceId")
		for _, row := range f.presence[partitionId] {
...
func (f *fakeRoomDeleter) DestroyRoom(_ context.Context, partitionId string) error {
	if f.err != nil {
		return f.err
	}
	f.deleted = append(f.deleted, partitionId)
	return nil
}
...
func newFixture(prefs map[string]any) *fakeEngine {
	...
	return &fakeEngine{
		...
		participants: map[string][]map[string]any{
			oldSpaceRowId: {
				{
					"id": "human-1", "partitionId": oldSpaceRowId, "userId": testUserId,
					"participantType": "human", "displayName": "Jose", "status": "active",
				},
				{
					"id": "si-ga-1", "partitionId": oldSpaceRowId, "agentId": "sofia",
		...
		presence: map[string][]map[string]any{
			oldSpaceRowId: {
				{
					"id": "presence-si-ga-1", "partitionId": oldSpaceRowId,
...
func TestSweep_ClosesOutRolledOverDaily(t *testing.T) {
	...
	if len(eng.leaveCalls) != 2 {
		t.Fatalf("expected 2 leaveSpace calls, got %d", len(eng.leaveCalls))
	}
	for _, call := range eng.leaveCalls {
		...
		if payload["partitionId"] != oldSpaceRowId {
			t.Errorf("participant payload partitionId = %v, want %s", payload["partitionId"], oldSpaceRowId)
		}
	}
	...
	if len(eng.rolloverWrites) != 1 {
		t.Fatalf("expected 1 rolloverDailySpace call, got %d", len(eng.rolloverWrites))
	}
	rw := eng.rolloverWrites[0]
	if rw["partitionId"] != oldSpaceRowId {
		t.Errorf("rollover partitionId = %v, want %s", rw["partitionId"], oldSpaceRowId)
	}
	...
	for _, q := range eng.queries {
		if strings.HasPrefix(q, "spaceParticipants") && strings.Contains(q, "daily-today") {
...
// TestPartitionIdForms pins the canonical/bare duality the sweep queries
// and deletes under.
func TestPartitionIdForms(t *testing.T) {
	got := partitionIdForms(oldSpaceRowId)
	if len(got) != 2 || got[0] != oldSpaceRowId || got[1] != oldSpaceBareId {
		t.Errorf("partitionIdForms(canonical) = %v", got)
	}
	got = partitionIdForms(oldSpaceBareId)
	if len(got) != 2 || got[0] != oldSpaceBareId || got[1] != oldSpaceRowId {
		t.Errorf("partitionIdForms(bare) = %v", got)
	}
}
...
func TestSplitCallHelper(t *testing.T) {
	name, args := splitCall(t, `leaveSpace({"participantId":"p1","payload":{"status":"left"}})`)
	if name != "leaveSpace" || args["participantId"] != "p1" {

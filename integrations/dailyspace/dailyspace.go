...
//	                            tick; the underlying createDailySpace
...
func (d *Integration) Capabilities() []memql.IntegrationCapability {
	return []memql.IntegrationCapability{
		{
			Name:        "ensureForUser",
			Description: "Ensure today's daily space exists for the given user. Reads the user's preferences.timezone to compute their local date key; calls createDailySpace idempotently on a content-addressed deterministic id derived from (kind=daily, userId, dateKey) via core/id.Engine.MustFromMap. No-op when preferences.dailySpaceEnabled is false. Safe to call repeatedly -- the underlying insert collapses on id collision.",
...
func (d *Integration) ensureForUser(ctx context.Context, userId string) (ensureResult, error) {
	...
	mutation := fmt.Sprintf(
		`createDailySpace({partitionId: %q, name: %q, dailyDateKey: %q, ownerUserId: %q})`,
		partitionId, name, dateKey, userId,
	)
...
// listActiveUsers runs activeUsers({}) and returns the flat row
...
func (d *Integration) listActiveUsers(ctx context.Context) ([]map[string]any, error) {
	raw, err := d.engine.Execute(systemActorContext(ctx), `activeUsers({})`)
	if err != nil {
		return nil, err
	}
...
// (userById, activeUsers, ...) silently returned 0 rows

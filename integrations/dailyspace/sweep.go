...
//  3. delete the polyphon-<spaceId> LiveKit room via the room-provider's
...
//     also the sweep's idempotency marker -- queryActiveDailySpacesForUser
//     filters on traitStatusIsActive + traitIsActiveRecord, so a swept
//     space never comes back.
//
// Cluster safety mirrors how rollover itself is guarded: pure
// idempotency, no lease. mutationCreateDailySpace is safe to race
...
func (d *Integration) sweepRolledOverDailies(ctx context.Context, userId, todayKey string, prefs map[string]any, stats *sweepStats) {
	...
	rows, err := d.queryRows(ctx, "queryActiveDailySpacesForUser", map[string]any{"userId": userId})
	if err != nil {
...
func (d *Integration) sweepSpace(ctx context.Context, userId string, space map[string]any, prefs map[string]any, stats *sweepStats) {
	...
	// status="left" version).
	for _, sid := range spaceIdForms(spaceRowId) {
		parts, err := d.queryRows(ctx, "querySpaceParticipants", map[string]any{
			"spaceId": sid,
			"status":  "active",
		})
		if err != nil {
		...
		for _, p := range parts {
			...
			payload["status"] = "left"
			payload["leftAt"] = now
			if err := d.execMutation(ctx, "mutationLeaveSpace", map[string]any{
				"participantId": pid,
				"payload":       payload,
			}); err != nil {
	...
	for _, sid := range spaceIdForms(spaceRowId) {
		rows, err := d.queryRows(ctx, "querySpaceParticipantPresence", map[string]any{"spaceId": sid})
		if err != nil {
		...
		for _, row := range latestPresenceByParticipant(rows) {
			...
			if err := d.execMutation(ctx, "mutationUpdateParticipantPresence", args); err != nil {
	...
	if err := d.execMutation(ctx, "mutationRolloverDailySpace", map[string]any{
		"spaceId": spaceRowId,
		"payload": payload,
	}); err != nil {

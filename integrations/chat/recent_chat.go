...
func (c *Integration) handle(ctx context.Context, args map[string]any, _ int) ([]memorynodes.MemoryNode, error) {
	...
	if spaceId == "" {
		return nil, fmt.Errorf("chat.recentChat: spaceId required")
	}
	...
	case "readRecent":
		...
		return c.readRecent(ctx, spaceId, count)
	case "readByKeyword":
		keyword := strings.TrimSpace(asString(args["keyword"]))
		if keyword == "" {
			return nil, fmt.Errorf("chat.recentChat.readByKeyword: keyword required")
		}
		return c.readByKeyword(ctx, spaceId, keyword)
	case "readByTime":
		from := strings.TrimSpace(asString(args["fromTime"]))
		to := strings.TrimSpace(asString(args["toTime"]))
		return c.readByTime(ctx, spaceId, from, to)
	case "getSpaceContext":
		return c.getSpaceContext(ctx, spaceId)
	case "listParticipants":
		return c.listParticipants(ctx, spaceId)
	default:
		return nil, fmt.Errorf("chat.recentChat: unknown operation %q", op)
	}
}

func (c *Integration) readRecent(ctx context.Context, spaceId string, count int) ([]memorynodes.MemoryNode, error) {
...
func (c *Integration) readByKeyword(ctx context.Context, spaceId, keyword string) ([]memorynodes.MemoryNode, error) {
	var nodes []memorynodes.MemoryNode
	err := c.bun().NewSelect().
		Model(&nodes).
		Where("concept = ?", memorynodes.ConceptCognitionUtterance).
...
func (c *Integration) readByTime(ctx context.Context, spaceId, from, to string) ([]memorynodes.MemoryNode, error) {
	q := c.bun().NewSelect().
...
func (c *Integration) getSpaceContext(ctx context.Context, spaceId string) ([]memorynodes.MemoryNode, error) {
	...
	return []memorynodes.MemoryNode{{
		ID:        fmt.Sprintf("recentChat:context:%s", spaceId),
...
func (c *Integration) listParticipants(ctx context.Context, spaceId string) ([]memorynodes.MemoryNode, error) {
	var nodes []memorynodes.MemoryNode
	err := c.bun().NewSelect().
		Model(&nodes).
	...
	bytes, _ := json.Marshal(out)
	return []memorynodes.MemoryNode{{
		ID:        fmt.Sprintf("recentChat:participants:%s", spaceId),
		Concept:   recentChatKind,
		Type:      memorynodes.NodeTypeObject,
		CreatedAt: time.Now().UTC(),
		Payload:   bytes,

...
// DSL for the cloud-engine avatar vendors -- Anam (avatar bring-up, #291)
// and Simli (memql#782).
//
...
//
//	avatardirect.startSession({ agentId: string, partitionId?: string })
//	    -> { livekit_url, livekit_client_token, browser_identity, vendor, room_name }
//	avatardirect.engageVendor({ agentId, room_name, browser_identity, partitionId? })
...
// Registered as a core plug-in (init() in plugin.go) so the bff node
	...
	return []memql.IntegrationCapability{
		{
			...
			ArgsSchema: map[string]string{
				...
				"partitionId": "string (optional) - originating space, for logging/correlation",
			},
		},
		{
			Name:        "engageVendor",
...
// persona catalog rows (v1:agents:avatarPersona). The client persona picker
...
//
//   - Agent stamped with a CATALOG row id -> hydrate via avatarPersonaById
...
//
// The agent lookup uses the named agentById (the agentFull shape projects
...
func (i *Integration) resolveAgentAvatar(ctx context.Context, agentId string) (vendor, personaId string, err error) {
	...
	q := fmt.Sprintf(`agentById({agentId: %s})`, string(agentIdJSON))
...
func (i *Integration) hydrateCatalogPersona(ctx context.Context, agentId, catalogId string) (string, string, error) {
	...
	raw, err := i.engine.Execute(systemActorContext(ctx), fmt.Sprintf(`avatarPersonaById({avatarPersonaId: %s})`, string(catalogIdJSON)))
	if err != nil {
...
func (i *Integration) defaultCatalogPersona(ctx context.Context, agentId, gender string) (string, string, error) {
	raw, err := i.engine.Execute(systemActorContext(ctx), `avatarPersonas({})`)
	if err != nil {
...
// mintBrowserToken builds the LiveKit join token the browser connects with. The

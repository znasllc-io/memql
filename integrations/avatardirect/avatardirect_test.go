...
// are not on this path). It dispatches per named query: agentById serves
// the agent's avatar fields; avatarPersonas / avatarPersonaById
...
func (f *fakeEngine) Execute(_ context.Context, query string) (*memql.ExecuteResult, error) {
	...
	if strings.Contains(query, "avatarPersona") {
		byId := strings.Contains(query, "avatarPersonaById")
...
func TestStartSession_Phase1MintsCredsWithoutEngine(t *testing.T) {
	...
	nodes, err := i.handleStartSession(context.Background(), map[string]any{"agentId": "v1:agents:agent:abc", "partitionId": "sp1"}, 0)
	...
	// The lookup used the named agentById (no raw DSL).
	assert.Contains(t, eng.gotQuery, "agentById")
...
func TestStartSession_StampedCatalogIdHydratesToVendorFaceId(t *testing.T) {
	// The client persona picker stamps the CATALOG ROW ID onto the agent;
	// Simli needs the vendor faceId. The resolution must hydrate through
	// avatarPersonaById instead of passing the catalog id verbatim as

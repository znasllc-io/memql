...
- Knowledge tab (assistant role ONLY): createAgent.domain.business-administration ("Business Administration", the org-wide baseline domain; locked-on for the GA. Specialists do NOT have this locked -- the row is opt-in like any other domain).
- Skills tab, capabilities sub-tab (locked for EVERY role -- the universal agent toolkit, six entries): createAgent.capability.data-query, createAgent.capability.document-search, createAgent.capability.calendar-access, createAgent.capability.notifications, createAgent.capability.email-compose, createAgent.capability.task-management.
- Skills tab, integrations sub-tab (locked for Assistant role ONLY; specialists do not get it locked and can opt in): createAgent.integration.copresent-control (ties the GA to the CoPresent Control feature so she can drive the UI on the user's behalf).

...
- Skills tab: the universal-toolkit skills are locked-on for every role -- createAgent.skill.<slug> rows that render with a lock glyph and cannot be removed (the baseline every agent carries). createAgent.skill.copresent-control is locked-on for the Assistant role ONLY (ties the GA to the CoPresent Control feature so she can drive the UI); specialists can opt into it. (There is NO Knowledge tab in this modal -- the org-wide baseline knowledge domain is managed in Training, not here; createAgent.domain.<slug> / createAgent.capability.<slug> / createAgent.integration.<slug> do NOT exist.)

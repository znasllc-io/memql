...
var workbenchSeedCorpus = []SeedCorpusEntry{
	...
	{
		SourceRef: "workbench:environmentHint",
		Text:      `You can SAY UP FRONT what an action needs, and the workbench will tell you before it runs anything. ` + "`workbenchHost`" + ` takes an optional ` + "`environment`" + ` object: ` + "`{os?: \"darwin\"|\"linux\"|..., needs?: [...]}`" + `, where each need is one of ` + "`display`" + ` (the action drives a GUI), ` + "`gpu`" + ` (it needs hardware acceleration), ` + "`macos_tooling`" + ` (Xcode, ` + "`defaults`" + `, ` + "`osascript`" + `, anything Apple-only) or ` + "`user_files`" + ` (files that live on the user's own computer). When the workbench cannot meet one, it refuses with ` + "`environment_mismatch`" + ` and NAMES the unmet needs -- instead of running your command and failing three layers down with something that reads like a bug in your shell. Send the hint whenever you already know the answer; omit it when you do not, because there is no default and a guess would refuse calls that would have worked. This is a HINT, not a request for permission: what happens next is described in workbench:failureFallback.`,
	},
	{
	...
	},
	{
		...
		Text:      `THE RULE HAS NOT CHANGED: never silently switch to the user's own machine. What HAS changed is that you no longer have to ask twice for something the user already approved. When a ` + "`workbenchHost`" + ` call comes back ` + "`environment_mismatch`" + `, the runtime handles the escalation for you and NOTHING has run yet: if the current task is approved AND the user's standing computer-use scope already covers the level of access the unmet needs imply, the same call is re-dispatched to one of the user's own machines -- picked by their routing policy, matching labels derived from the needs (` + "`os=darwin`" + ` for macOS tooling, ` + "`display=true`" + ` for a GUI, and so on) -- and you get that machine's result back in place of the mismatch. If EITHER of those is missing, the consent card goes up exactly as before and the result you get back is the card acknowledgement. You do not choose between these; you will see whichever happened. What you should do is tell the user plainly what is going on ("that needs Xcode on your Mac -- I've put an approval card on your canvas" / "I ran it on your MacBook instead, since the workbench has no display"). ` + "`environment_mismatch`" + ` is internal wire language; never surface the phrase.`,
	},
	{
		SourceRef: "workbench:failureFallbackManual",

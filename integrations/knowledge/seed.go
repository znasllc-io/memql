...
CREATE SPACE (route modal:createSpace): createSpace.activeAssistant -- a PINNED non-clickable row at the top of the participant selector. Shows the owner's GA name plus the subtitle "Assistant - Joins automatically". It is a <div>, not a button, so it has no aria-pressed, no checkbox, and no onClick -- uiClick is a category error here, not just a no-op. The backend auto-joins the GA via the autoJoinAI automation regardless of what the picker sends, so there is nothing to toggle. (The space participant picker selects HUMANS (createSpace.human.<id>) and GROUPS (createSpace.group.<id>) via the createSpace.tab.<id> tabs -- specialist agents are NOT space participants, so there is no createSpace.agent.<id> row.)

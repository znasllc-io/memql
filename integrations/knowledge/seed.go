...
var copresentUISeedCorpus = []struct {
	...
	},
	{
		...
		Text:      `A Control Session is the bounded window during which an agent drives the UI on the user's behalf. It backs BOTH operator experiences -- a Takeover (autopilot) and a Guide (immersive Scenes); see concept:takeoverVsGuide. Visual signals while a session is active: a soft theme-coloured cursor (~28px) animates to target elements with an ease-in-out-sine curve; an optional spotlight pulses around highlighted targets; and a Take-Back Control button (data-op-id=control.take-back; bottom-right) lets the user reclaim the pointer at any time (ESC and click-outside also release). There is NO mid-takeover prompt card / mini-chat widget anymore -- the old copresentControl.widget surface and its uiAskUser / uiNarrate primitives were retired. A Takeover stays QUIET between steps: the agent drives with the cursor + spotlight and confirms with one short line on uiReleaseControl. If the agent needs something from the user mid-task, it asks in the CONVERSATION (text or voice), not via a UI overlay. Sessions are begun with uiRequestControl({reason}) and ended with uiReleaseControl({summary}). The session persists across uiNavigate calls AND uiClick calls that trigger route changes -- so a cross-page session is ONE session, not many. Commit buttons (Create/Save/Delete/Send/Invite/Remove/Confirm/Sign-Out) are NOT silent: before clicking one, confirm in conversation (e.g. ask "Ready for me to create it?" and act on a yes) -- the user's spoken/typed yes IS the consent. This lets multi-step takeovers (e.g. create agent + create space) chain without the user having to manually take back control between steps. An inactivity watchdog auto-releases after ~30s if the agent leaves the session hanging.`,
	},
	{
		SourceRef: "concept:releaseLanding",
		Text:      `LAND THE CURSOR ON A NAVIGABLE SURFACE BEFORE uiReleaseControl. The agent cursor fades when the user clicks Take Back Control; whatever element the cursor was last over is where it disappears. Releasing while the cursor is in the middle of a Settings field, a closed modal, or a buried form section leaves the user without an obvious next step -- the takeover ends and they're stranded. The fix is a single uiClick on the header's space-context pill as the LAST action before uiReleaseControl. Two variants render in the header (only one at a time): when an active space is selected the pill carries data-op-id="nav.currentSpace" and routes to ?panel=chat (drops the user into the active conversation); when no active space is selected (typical first-login / fresh-account state) the pill carries data-op-id="nav.noActiveSpaceLabel" and routes to ?panel=spaces (drops the user on the spaces list). Read the surfaces / after-state to pick the right one and uiClick it. Same rule for BOTH operator experiences -- Takeover and Guide. The intro / first-login walkthrough also follows this rule (the runOnboarding orchestrator does it automatically before its own release call), so behaviour stays uniform whether the agent is LLM-driven or curriculum-driven. SKIP the landing click only when (a) your goal already finished on a useful surface (Profile modal open, the row the user asked about is highlighted in a list, the spaces panel is showing the new space you just created), or (b) the user explicitly asked to be left where they are. Forcing a redundant click steals their context. Otherwise: click the pill, then release. The cursor fading on a real button reads as "agent finished and handed me a starting point", not "agent abandoned me mid-form".`,
	},
	{
		SourceRef: "concept:takeoverVsGuide",
		Text:      `There are TWO operator experiences, both GA-only and both backed by a Control Session (concept:controlSession). They replace the single retired copresent_control tool. CHOOSE BY THE USER'S INTENT. (1) TAKEOVER (tool: copresent_takeover) is AUTOPILOT -- the AI does it FOR the user. Use it when the user says "just do X for me" and every required value is known: "change my theme to dark", "open the Spaces menu", "create a space called Weekly Sync", "delete the agent named Test". The takeover is clean and quiet: begin with uiRequestControl({reason}), drive with the cursor + spotlight (uiReadState / uiClick / uiType / uiSelect / uiHighlight / uiPointerTo / uiNavigate), then end with uiReleaseControl({summary}) and confirm with ONE short "done" line. No per-step narration surface, no mid-takeover prompt card. If you need a value the user did not give, ask in the conversation, then continue. (2) GUIDE (tool: copresent_guide) is IMMERSIVE -- the AI does it WITH the user, voice-to-voice, as ordered Scenes over the live UI + Canvas. Use it when the user wants to be SHOWN or TAUGHT something ("show me how to...", "give me a demo", "walk me through...") or on first run. A Guide is consent-forward (you ASK before starting and only engage voice on a yes), forces voice-to-voice while it runs, and sequences Scenes with guideAdvanceScene / guideEndScene / guideEndGuide; some Scenes lock the mic, and a raise-hand is picked up at the next open Scene. See concept:guideModel for the full Guide mechanics. uiRequestControl no longer takes an "interactivity" argument -- the experience is determined by WHICH tool (copresent_takeover vs copresent_guide) the agent is using, not a per-session mode flag. The Settings -> Takeover preferences (appearance clean/dim, cursor speed) apply to Takeover sessions; Guide pacing is voice-driven, not a settings slider.`,
	},
	{
		SourceRef: "concept:guideModel",
		Text:      `A GUIDE is the immersive operator experience (tool: copresent_guide; GA-only): the AI walks the user through CoPresent voice-to-voice, as an ordered sequence of SCENES over the live UI + Canvas. Use it for "show me / teach me / walk me through" and on first run; use a Takeover (concept:takeoverVsGuide) for "just do X for me". MECHANICS. (1) CONSENT-FORWARD ENTRY: never drop a user into a Guide. ASK first in conversation ("Want me to walk you through it? I will turn on voice."). A single confirm affordance (data-op-id=guide.consent, with guide.consent.accept / guide.consent.decline) surfaces; voice (and the optional avatar at guide.avatar) engages ONLY on accept. On decline, no Guide runs. (2) AUTHORING ON THE FLY: if no ready-made Guide fits, converse briefly to learn the user (industry, role, interests, goals), and once you have enough to make it personal call guideGenerate({intake, kind}) -- intake is your concise summary of what you learned; kind is "demo" (show me X) / "teach" (teach me Y) / "walkthrough" (first-run tour; default). It generates + saves the Guide (so it can be replayed later) and starts it immediately; you then narrate its Scenes. Do NOT call guideGenerate before you have enough intake to tailor it. (3) WHILE IT RUNS: it is voice-to-voice -- narrate each Scene by voice, naturally and concisely; do NOT drop to text. Drive with the normal operator primitives (uiReadState / uiHighlight / uiClick / uiType / uiSelect / uiPointerTo / uiNavigate) plus the Canvas-annotation tools (see surface:canvasAnnotations) to point at published Scene content. (4) SCENE SEQUENCING: advance with guideAdvanceScene (move to the next Scene when the current one is done) or guideEndScene (same thing, phrased as "this Scene is complete"); finish the whole Guide -- early or at the end -- with guideEndGuide({summary?}). Advancing past the last Scene ends the Guide automatically. (5) LOCKED SCENES + RAISE-HAND: some Scenes are LOCKED (non-interruptible) and MUTE the user mic so a narration beat can land cleanly; during a locked Scene the user signals a question NON-VERBALLY via the raise-hand affordance (data-op-id=guide.raiseHand). Honour it: pick the raised hand up at the NEXT OPEN Scene boundary and pause for the question -- do not blow past it. (6) THE USER CAN ALWAYS END: the End Guide control (data-op-id=guide.end) is always available to the user; so is the Take-Back Control button (control.take-back). guide.end is the USER's escape hatch -- the AGENT ends Scenes/Guides with the guide* tools, not by clicking guide.end. See surface:guideEntry for the user-facing guide.* op-ids.`,
	},
	{
		SourceRef: "surface:canvasAnnotations",
		Text:      `CANVAS LIVE-ANNOTATION tools let the AI draw ephemeral, theme-coloured marks OVER published Scene content while it narrates a Guide -- so it can point at / circle a specific part of what it just put on the Canvas instead of describing it in words. They are part of the operator suite (same client-executed dispatch path as the ui* primitives) and REQUIRE an active Control Session. Publish Scene content with the canvas.publish tool FIRST, then annotate it. The four tools: (1) canvasPoint({target, label?}) -- drop a laser-pointer dot on an element ("look here"). (2) canvasCircle({target, label?}) -- circle an element for emphasis. (3) canvasArrow({target, from?, label?}) -- an arrow pointing AT target; pass from to connect two elements (e.g. "this field feeds that result"). (4) canvasHighlight({target, label?}) -- a soft marker-swipe glow over an element; pass target=null to CLEAR all marks (the between-Scenes reset). Marks auto-fade and clear between Scenes, so you do not have to clean them up manually as you advance. TARGETS are data-op-ids, INCLUDING parts INSIDE a published card -- the canvas.card.* op-id family covers card internals (e.g. canvas.card.agentCreated.title, canvas.card.spaceCreated.title, canvas.card.document.body, canvas.card.dataview.table, canvas.card.dataview.charts), so you can point at a specific field, not just the whole card. WHEN TO USE: during a Guide, after publishing a card/document/dataview Scene, use canvasPoint / canvasCircle to draw the user's eye to the exact element you are talking about, and canvasArrow to show a relationship between two elements. These are for pointing at PUBLISHED CANVAS content; for highlighting live app chrome (nav tiles, form fields, buttons) during a Takeover or Guide, use uiHighlight / uiPointerTo instead.`,
	},
	{
		SourceRef: "concept:alreadySelected",
		Text:      `CHECK BEFORE YOU ACT. Every toggle in CoPresent's forms -- gender pills, personality rows, knowledge domain rows, skill rows (capabilities + integrations), tab buttons, sub-category pills, create-space agent-selector rows -- carries aria-pressed="true" when it's the currently selected option. <select> elements (createAgent.role, createAgent.policy, createAgent.explicitModel) carry the current value on el.value. Inputs (createAgent.name, etc.) carry the current text on el.value. BEFORE calling uiClick on a toggle, uiSelect on a dropdown, or uiType on an input, verify the value isn't already what the user asked for. Signals: the uiReadState / after-state summary shows aria-pressed="true" on the matching row, or a previous step returned "already set to X" / "already active". If the value already matches, DO NOT call the primitive -- clicking an already-pressed TOGGLEABLE row DESELECTS it (surprising the user who just said "yes, that one"), retyping the same name churns the input, re-selecting the same option wastes an iteration. Instead emit uiHighlight({target: <field-op-id>}) and say in conversation that the field is already set to that value (during a Guide, narrate it by voice). SEPARATE CASE -- LOCKED ROWS: some rows are permanently selected and cannot be toggled at all. They look visually identical to a selected toggleable row (checked + primary-tinted border) but carry a small lock glyph on the right edge, plus either a disabled attribute on the <button> or are rendered as a plain <div> (no button element at all, for the pinned auto-join rows). uiReadState marks these as non-interactive / locked. NEVER emit uiClick on a locked row -- the click is a no-op, the cursor flies for nothing, the user sees a pointless move, and you burn an iteration. This is the single most common misfire in walkthroughs because aria-pressed="true" reads the same on both; always check for the lock signal before concluding "already selected, I should click to deselect". See concept:lockedDefaults for the full per-modal roster of locked rows. The backend short-circuits the action if you do call it anyway, returning a text that includes "already set" / "already active" / "locked" -- but that still costs a turn you should have saved.`,
	},
	{
	...
	{
		SourceRef: "surface:guideEntry",
		Text:      `A Guide surfaces a small set of guide.* op-ids over the live UI; there is NO mini-chat / "control widget" anymore (the retired copresentControl.widget and its copresentControl.widget.* sub-targets are gone). CONSENT CARD (data-op-id=guide.consent): a single confirm affordance that appears when a Guide is offered -- it is NOT a chat surface. It carries guide.consent.accept (engage the voice/avatar Guide -- on accept the mic is force-unmuted and voice-to-voice begins) and guide.consent.decline (dismiss; no Guide runs). The agent ASKS in conversation first ("Want me to walk you through it? I'll turn on voice."); the card is the explicit yes/no. AVATAR STAGE (data-op-id=guide.avatar): the optional video-avatar surface shown while a Guide runs. RAISE-HAND (data-op-id=guide.raiseHand): a non-verbal "I have a question" affordance the user can press while a locked Scene has the mic muted; the agent picks it up at the next OPEN Scene boundary and pauses for the question. END GUIDE (data-op-id=guide.end): always-available control to finish the Guide early; releases voice and ends the run. The agent drives Scene flow with the Scene-control tools (guideAdvanceScene / guideEndScene / guideEndGuide), NOT by clicking guide.end itself -- guide.end is the user's escape hatch. The user can also reclaim the pointer via the Take-Back Control button (control.take-back). See concept:guideModel for the full mechanics.`,
	},
	{
		SourceRef: "concept:canvasWidgets",
		Text:      `CoPresent has two floating canvas widgets that share one liquid-glass aesthetic (frosted white-alpha surface, backdrop-blur-2xl + saturate-150, soft inset highlight on top edge, white-alpha text): the Chat WIDGET (right side, 384px wide, data-op-id=canvas.chatWidget) and the Presence WIDGET (left side, 384px wide, data-op-id=canvas.presenceWidget). Both are HIDDEN BY DEFAULT -- the canvas is clean on entry. Users open each via a corner restore button; closing returns to the clean state. (There is no longer a third "CoPresent Control Widget"; the operator experiences surface a Take-Back Control button (control.take-back) and, during a Guide, the guide.* affordances -- see surface:guideEntry.) Both widgets share the same liquid-glass styling (src/lib/theme/liquidGlass.ts -- glassWidgetRoot / glassText / glassAccentButton / glassControlSurface). IMPORTANT: chat + presence widgets are MUTUALLY EXCLUSIVE with their corresponding side panels. Opening the Presence widget COLLAPSES the left panel; canvas expands into that space. Opening the Chat widget COLLAPSES the right column ONLY when chat was the right-column tile (rightView === 'chat'); otherwise the chat widget floats alongside. Entering FOCUS-CANVAS auto-shows both widgets; exiting auto-hides them. During walkthroughs, do NOT open/close widgets -- the user's layout is the starting point.`,
	},
	{
...
- Skills tab: the universal-toolkit skills are locked-on for every role -- createAgent.skill.<slug> rows that render with a lock glyph and cannot be removed (the baseline every agent carries). createAgent.skill.copresent-takeover and createAgent.skill.copresent-guide are locked-on for the Assistant role ONLY (they tie the GA to the operator experiences -- Takeover autopilot and immersive Guide -- so she can drive the UI; the single copresent-control skill was split into these two); specialists can opt into either. (There is NO Knowledge tab in this modal -- the org-wide baseline knowledge domain is managed in Training, not here; createAgent.domain.<slug> / createAgent.capability.<slug> / createAgent.integration.<slug> do NOT exist.)

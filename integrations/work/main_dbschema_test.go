...
//
// Adding this TestMain is what makes `integrations/work` a db-gated package,
// which obliges the db-tests lane selector in .github/workflows/ci.yml to
// name it -- scripts/cidb/dbgate_test.go asserts that in both directions
// (memql#2886), and it was the missing half here: the lane already named the
// package (epic memql#4966) while the package had no TestMain, which is the
// combination that fails INTERMITTENTLY rather than loudly.

...
//
//   logicRouteRequest / logicRecordTransition write
//   `mutationRecordRequestEvent({ requestId: args.event.payload.id, ... })`,
//   and a node.created / node.updated event's `payload.id` is the CANONICAL
//   node id (`v1:forge:request:r-...`). The TOOL path (logicRecordMentoring)
//   passes the SHORT tool arg, so its `mentored` event is keyed short. The
//   audit trail splits across two id forms; queryRequestEvents matches the
//   field literally, so a short-id read only finds one form.
//
// The fix normalizes on the WRITE side: `mutationRecordRequestEvent` stores
...
// queryRequestEvents(<shortId>) returns the full trail.
//
...
func runForgeAuditShortId(t *testing.T, e *Env) {
	...
	// mutationRecordRequestEvent therefore receives the canonical id -- which the
	...
	routedByShort := asArray(t, e.runQuery(t, "queryRequestEvents", map[string]any{"requestId": shortId}))
	if !hasEventKind(routedByShort, "routed") {
		t.Fatalf("#1859: queryRequestEvents(<shortId>=%q) did not return the 'routed' event "+
	...
	routedByCanonical := asArray(t, e.runQuery(t, "queryRequestEvents", map[string]any{"requestId": canonicalId}))
	if len(routedByCanonical) != 0 {
		t.Fatalf("#1859: queryRequestEvents(<canonicalId>=%q) returned %d event(s); expected 0 -- "+
	...
	afterTransition := asArray(t, e.runQuery(t, "queryRequestEvents", map[string]any{"requestId": shortId}))
	if !hasEventKind(afterTransition, "approved") {
		t.Fatalf("#1859: queryRequestEvents(<shortId>=%q) did not return the 'approved' event after the queued "+
			"transition (recordTransition keyed it under the canonical id); events=%v", shortId, afterTransition)
	}

	// --- C. tool path (logicRecordMentoring -> 'mentored', short id) ----------
	...
	e.runMutation(t, "mutationRecordRequestEvent", map[string]any{
		"requestId": shortId, // short tool arg, exactly as logicRecordMentoring passes it
		"kind":      "mentored",
		"note":      "conf-1859 mentoring touch",
	})

	full := asArray(t, e.runQuery(t, "queryRequestEvents", map[string]any{"requestId": shortId}))

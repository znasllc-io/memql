...
//
// queryDelegationsByIdentity is documented as returning "all delegations
// (active and revoked) for a given identity ID" -- it is the delegation-history
// audit query. It used to carry an `&& traitIsActiveRecord` predicate, which
...
// queryActiveDelegationsByIdentitySubject (the DelegationResolver hot-path
...
func TestConf1728DelegationsByIdentityIncludesRevoked(t *testing.T) {
	...
	byIdentity := asArray(t, e.runQuery(t, "queryDelegationsByIdentity", map[string]any{"identityId": identityID}))
	if !containsID(byIdentity, storedID) {
		t.Fatalf("#1728 pre-revoke: active delegation %s missing from queryDelegationsByIdentity; got %v", storedID, idIDs(byIdentity))
	}
	activeBefore := asArray(t, e.runQuery(t, "queryActiveDelegationsByIdentitySubject", map[string]any{"identitySubject": identitySubject}))
	...
	byIdentityAfter := asArray(t, e.runQuery(t, "queryDelegationsByIdentity", map[string]any{"identityId": identityID}))
	if !containsID(byIdentityAfter, storedID) {
		t.Fatalf("#1728 REGRESSION: queryDelegationsByIdentity dropped revoked delegation %s -- the active-only predicate is back; got %v", storedID, idIDs(byIdentityAfter))
	}

	// EXCLUSION (the contract that must NOT change): the active-only resolver
	// query no longer returns the revoked row.
	activeAfter := asArray(t, e.runQuery(t, "queryActiveDelegationsByIdentitySubject", map[string]any{"identitySubject": identitySubject}))
	if containsID(activeAfter, storedID) {
		t.Fatalf("#1728: queryActiveDelegationsByIdentitySubject leaked revoked delegation %s; got %v", storedID, idIDs(activeAfter))
	}

	t.Logf("#1728: revoked delegation %s included in queryDelegationsByIdentity (all-rows) and excluded from queryActiveDelegationsByIdentitySubject (active-only)", storedID)
}

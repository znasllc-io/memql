...
func seedRecord(t *testing.T, e *Env, recordId, spaceId, label string, data map[string]any) string {
	t.Helper()
...
func runLifecycle(t *testing.T, e *Env) {
	...
	got := asArray(t, e.runQuery(t, "queryRecordsByState", map[string]any{"spaceId": space}))
	...
	after := asArray(t, e.runQuery(t, "queryRecordsByState", map[string]any{"spaceId": space}))
...
func runFilter(t *testing.T, e *Env) {
	...
	res := asArray(t, e.runQuery(t, "queryRecordsByState", map[string]any{"spaceId": spaceX}))
	...
	none := asArray(t, e.runQuery(t, "queryRecordsByState", map[string]any{
		"spaceId": spaceX, "recordType": "no-such-type",
	}))
...
func runPartialUpdate(t *testing.T, e *Env) {
	...
	for _, f := range []string{"label", "recordType", "spaceId", "importSource", "data"} {
...
func runEnvelope(t *testing.T, e *Env) {
	...
	zero := e.runQuery(t, "queryRecordsByState", map[string]any{"spaceId": "space-empty-" + uniqueSuffix("env")})
	z := asArray(t, zero)
	if len(z) != 0 {
	...
	one := asArray(t, e.runQuery(t, "queryRecordsByState", map[string]any{"spaceId": oneSpace}))
	if len(one) != 1 {
	...
	many := asArray(t, e.runQuery(t, "queryRecordsByState", map[string]any{"spaceId": manySpace}))
	if len(many) != 3 {

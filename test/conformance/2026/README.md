...
| `files-memql-<language>` | The frozen edition's machine-readable case list (memql#5385 audit): every corpus file a case names, fixture.memql excluded, one per line, sorted, trailing newline, no header -- the set a second implementation replays to claim conformance with this edition, in the convention toml-test's `files-toml-1.0.0` uses. Generated; `test/conformance/corpus_file_list_test.go` (`TestCorpusFileListMatchesTheLanguageLine`) holds it to the corpus and names the `-update-file-list` flag that regenerates it. |

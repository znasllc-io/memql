...
func TestCallGraphContract(t *testing.T) {
	...
	findings, err := callgraph.CheckTree(dslPath(t, "."), callgraph.SideEffectClassifier(sideEffecting))
	if err != nil {
...
func TestCallGraphCoverage(t *testing.T) {
	coverage, err := callgraph.Coverage(dslPath(t, "."))
	if err != nil {
		t.Fatalf("walk DSL tree: %v", err)
	}
	if len(coverage) == 0 {
...
func declarationsInTree(t *testing.T, kind string) int {
	...
	count := 0
	err := filepath.WalkDir(dslPath(t, "."), func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}
		if d.IsDir() {

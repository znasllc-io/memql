...
//
// It also schedules the domain's UNREGISTRATION. The registry is a
// process-wide global, so a domain left mounted is visible to every
// later test in the binary -- and since memql#3242 moved this suite
// out of `dsl/` into one package (it was `package dsl_test` before,
// which Go happened to run after the internal tests), "later" now
// includes the tests that load the unified tree and would fail on the
...
func uniqueDomain(t *testing.T) string {
	...
	domain := strings.ReplaceAll(t.Name(), "/", "_")

...
func TestListPackDomains_EmbeddedCore(t *testing.T) {
	domains := ListPackDomains()
	if len(domains) == 0 {
		t.Fatal("expected at least one embedded domain")
	}

	byName := map[string]PackDomain{}
...
func TestPackBrowse_RegisteredPack(t *testing.T) {
	...
	RegisterTree(dom, pack)
	t.Cleanup(func() { UnregisterTree(dom) })
	...
	var found *PackDomain
	for _, d := range ListPackDomains() {
		if d.Name == dom {
			dd := d
			found = &dd
		}
	...
	// Read a file back.
	src, origin, err := ReadPackFile(dom, "queries.memql")
	if err != nil {
...
func TestReadPackFile_PathTraversalRejected(t *testing.T) {
	...
	for _, c := range cases {
		if _, _, err := ReadPackFile(c.domain, c.path); err == nil {
...
func TestListPackFiles_UnknownDomain(t *testing.T) {
	if files := ListPackFiles("definitely-not-a-domain"); len(files) != 0 {
...
func TestPackFileExists(t *testing.T) {
	if !PackFileExists("cognition", "queries.memql") {
		t.Error("cognition/queries.memql should exist")
	}
	if PackFileExists("cognition", "no-such-file.memql") {
		t.Error("nonexistent file reported as existing")
	}
	if PackFileExists("cognition", "../identity/concepts.memql") {

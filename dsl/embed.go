...
// / v1:curriculum:*).
//
//go:embed all:actions all:agents all:authoring all:calendar all:campaigns all:capabilities all:cluster all:cognition all:common all:data all:deployment all:forge all:harness all:healing all:identity all:integrations all:knowledge all:library all:memql all:notes all:observability all:planner all:platform all:policies all:portalviews all:providers all:rbac all:router all:safety all:telephony all:todos all:workbench all:worker

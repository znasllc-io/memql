...
//
//go:embed all:agents all:cluster all:cognition all:common all:curriculum all:data all:guide all:harness all:identity all:knowledge all:memql all:notes all:observability all:planner all:platform all:policies all:providers all:router all:safety all:todos all:workbench all:worker

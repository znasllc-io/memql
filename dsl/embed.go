...
//
//go:embed all:agents all:cluster all:cognition all:common all:curriculum all:data all:guide all:identity all:knowledge all:memql all:observability all:planner all:platform all:policies all:providers all:router all:safety all:workbench all:worker

...
// osOnlineRulePath is the MemQL OS shell's copy of the online rule, relative
// to this package (memql#4719). Its Fleet section and its provenance dots
// decide `online` per row while rendering and cannot ask the engine per row,
// so they restate the same literal. The path is written out rather than
// derived so a failure names the file a reader has to go and open.
//
// TWO implementations, one number. It was three until epic memql#4984 retired
// the portal, whose clients/portal/src/fleet/online.ts carried a third copy.
const osOnlineRulePath = "../../clients/os/src/apps/fleet/online.ts"

// onlineWindowPattern matches the exported window constant and captures
...
var onlineWindowPattern = regexp.MustCompile(
	`(?m)^\s*(?:export\s+)?(?:const|let|var)\s+ONLINE_WINDOW_SECONDS\s*(?::[^=]+)?=\s*(\d+)\s*;?\s*$`,
)

// TestFleetOnlineWindowMatchesTheClients keeps the two implementations of the
// online rule in step.
//
// THERE ARE DELIBERATELY TWO. component/worker.IsOnline is the rule for
// anything server-side; clients/os/src/apps/fleet/online.ts is the rule for
// the Fleet section, which decides it per row while rendering and cannot ask
// the engine per row. Neither can be deleted in favour of the other, and the
// DSL cannot carry it either -- `online` is not projectable, because a shape
// body is a path list and this is a predicate over two timestamps and a clock.
//
...
// vacuous -- it would pass forever the moment a client's file were renamed or
// moved, which is exactly when the two rules are most likely to have diverged.
// The failure names the path so the fix is obvious in either direction. (That
// property is why retiring the portal reached this file at all: the third copy
// could not simply stop existing.)
func TestFleetOnlineWindowMatchesTheClients(t *testing.T) {
	for _, path := range []string{osOnlineRulePath} {
...
func assertOnlineWindowMatches(t *testing.T, rulePath string) {
	...
	m := onlineWindowPattern.FindSubmatch(raw)
	if m == nil {

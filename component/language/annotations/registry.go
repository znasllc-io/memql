...
var ByReceiver = map[string][]string{
	...
	"Logic": {
		"description", "enabled", "disabled", "entrypoint", "eventField",
	},
	"Automation": {
...
var Docs = map[string]string{
	...
	"eventField":  "On an event-triggered logic: declare the allowed top-level event payload fields (e.g. @eventField(\"spaceId\", \"siParticipantId\")). Opt-in field-level validation -- every event.payload.<field> reference in the body is checked against this set at load time, rejecting typos / fields the (possibly synthetic, handler-assembled) triggering event cannot carry (memql#1743). Bare names or payload.-prefixed paths both normalize to the head segment.",

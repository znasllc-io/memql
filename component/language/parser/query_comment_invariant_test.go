...
// query_comment_invariant_test.go -- memql#2948.
//
...
// for the query lane too. What it did not reach were the query lane's OWN
// locators and a second and unrelated comment-blindness. (The logic and
// automation lanes are read by the statement parser now, epic memql#5370;
// their sweep is comment_invariant_statement_test.go.)
//
...
//	B. parseStructQueryBody's inner `args {` locator ran its regexp on RAW
//	   source while matchBraceInBody matches on the BLANKED view. The two can
//	   never agree about a header inside a comment: the header IS in the raw
//	   text, its `{` is NOT in the blanked view -- so the error named a block
...
// because nothing surfaces it. compiledForm strips comments and collapses
...
// compiledForm is a rewriter output with its comments blanked and its
// whitespace collapsed: two outputs that differ only in comments compare
// equal.

...
// four where the migration is NOT new: wire_bare_ids_test.go:234 already booted
...
// wire_bare_ids_test.go:234 is unchanged and still runs; only the migration it
// performs is redundant now, because TestMain got there first. The tests
// already wrote and deleted rows when a reachable database was also migrated;
// lane membership only ever governed CI.
//

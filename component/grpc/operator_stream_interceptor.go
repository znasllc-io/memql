...
// present `Authorization: Operator <key>` matching auth.EnvOperatorKey
...
func NewOperatorAwareStreamInterceptor(
	...
	return func(srv interface{}, ss grpc.ServerStream, info *grpc.StreamServerInfo, handler grpc.StreamHandler) error {
		...
		expected := strings.TrimSpace(os.Getenv(auth.EnvOperatorKey))
		if expected == "" {
			if logger != nil {
				logger.Warn("operator auth: rejected -- " + auth.EnvOperatorKey + " not configured on this node")
			}
		...
		if len(expected) < minOperatorKeyLen {
			...
			if logger != nil {
				logger.Warn("operator auth: rejected -- configured "+auth.EnvOperatorKey+" is too short to be a credential",
		...
		if subtle.ConstantTimeCompare([]byte(got), []byte(expected)) != 1 {
			if logger != nil {
				logger.Warn("operator auth: rejected -- operator-key mismatch")
			}
		...
		claims := map[string]any{
			...
			OperatorAuthClaimKey: map[string]any{
				// Names the credential that admitted the stream. Was
				// "master_key" until memql#3519 split the two; an audit log
				// that still said master_key would now be simply wrong.
				"source": "operator_key",
			},
		}

...
// directly"). Every guest-invitation mutation (create, accept, kick, rotate,
// the guest participant) and both auth-session mutations (create, revoke)
// went through it, so all of them had been failing at parse in production,
...
// rendered statement through the real parser is
// render_query_args_parse_test.go, the same shape as
// deploy_control_parse_test.go, which found the identical defect in
// component/deploycontrol (memql#4209).

...
func (s *Service) RegisterRoutes(mux *http.ServeMux) {
	...
	// The preflight a cross-origin fetch sends first (memql#4624). Without it
	// a webview never reaches the GET.
	mux.Handle("OPTIONS /.well-known/memql-config.json", DiscoveryHandler(s.cfg, os.Getenv))
	...
	mux.Handle("OPTIONS /.well-known/oauth-authorization-server", OAuthServerMetadataHandler(s.cfg))

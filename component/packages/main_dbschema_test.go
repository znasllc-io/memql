...
// This package joined the lane in epic memql#4885, with credentials_db_test.go
// -- the owner-scoped credential resolution that only a real read gate over
// real rows can measure.
//
// When no DB is reachable EnsureSchema is a no-op and the individual tests
// self-skip, so a developer without Postgres sees exactly what they did
// before; MEMQL_REQUIRE_DB=1 turns that skip into a failure.

...
func (e *MemQLEngine) run(ctx context.Context, markStarted func()) error {
	...
	// Phase 0 of the llm-driven-decisions plan (see
	// docs/internal/planning/llm-driven-decisions.md): each cache logs its
	// hit/miss/eviction stats every 5 minutes so we can baseline
	// real-world cache effectiveness before migrating decisions to
	// LLM+cache. Emitters stay silent when the cache hasn't been touched --
	// no log noise on a quiet system.

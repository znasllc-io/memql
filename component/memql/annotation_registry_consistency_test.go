...
func receiverFixture(r annotations.Receiver, name, ann string) string {
	...
	case annotations.Automation:
		// The retired body form until the parser refuses it: the registry
		// still places @schedule and @trigger's partition key on an
		// automation, and a statement body refuses both (D15). The two go
		// together when the retired forms are refused.
		// memqlmigrate:keep
...
func runReceiverGate(r annotations.Receiver, src string) error {
	...
	tree := fstest.MapFS{
		"support/concepts.memql": {Data: []byte(src)},
...
func TestRetiredAndMisplacedReachTheRealGates(t *testing.T) {
	cases := []struct {
		...
		{annotations.Action, `@sideEffect("write")`, annotations.CodeRetired, "lives on the capability declaration"},
		{annotations.Action, `@kind("primitive")`, annotations.CodeRetired, "a composite of several is an automation"},

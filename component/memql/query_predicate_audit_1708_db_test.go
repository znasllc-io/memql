...
//   queryAvatarPersonas        -- hardcoded `vendor=="simli"` dropped every other
//                                 vendor's personas. Fixed to an optional vendor arg.
//   queryUsersScheduledForDeletion / queryUsersInDeletionCooldown -- must return
//                                 ONLY users with deletionScheduledAt set, not
//                                 every active user.
// Plus #1714:
//   queryExpiredConsumedAuthCodes -- name says Consumed; filter now gates on
...
func TestQueryAvatarPersonas_VendorOptionalNotHardcoded(t *testing.T) {
	...
	runMutation(t, ctx, eng, "mutationCreateAvatarPersona", map[string]any{
	...
	all := queryIds(t, ctx, eng, "queryAvatarPersonas()")
	...
	require.False(t, contains(all, inactiveID),
		"inactive persona MUST be excluded (traitIsActiveRecord), got %v", all)

	// Optional vendor scoping still works: vendor=simli excludes anam.
	simliOnly := queryIds(t, ctx, eng, `queryAvatarPersonas({"vendor":"simli"})`)
	...
	anamOnly := queryIds(t, ctx, eng, `queryAvatarPersonas({"vendor":"anam"})`)
...
func TestQueryUsersScheduledForDeletion_OnlyScheduled(t *testing.T) {
	...
	runMutation(t, ctx, eng, "mutationScheduleAccountDeletion", map[string]any{"userId": scheduledID})

	for _, q := range []string{"queryUsersScheduledForDeletion()", "queryUsersInDeletionCooldown()"} {
...
// --- #1714: queryExpiredConsumedAuthCodes gates on consumedAt ---------------
...
func TestQueryExpiredConsumedAuthCodes_OnlyConsumed(t *testing.T) {
	...
	seedCode := func(id string) {
		runMutation(t, ctx, eng, "mutationCreateAuthCode", map[string]any{
	...
	runMutation(t, ctx, eng, "mutationConsumeAuthCode", map[string]any{"codeId": consumedID})

	got := queryIds(t, ctx, eng, `queryExpiredConsumedAuthCodes({"now":"2026-06-18T00:00:00Z"})`)

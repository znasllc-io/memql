...
// TestTokenizeEqualityOperator was retired in #328 alongside the
// recursive-descent parser's `tokenize` + `tokenType` types. The
// langparser's own lexer tests (component/language/parser/lexer_test.go)
// cover the same surface from the langparser side; the engine-side
// guarantee (parse-then-shape) is exercised by
// `TestParseBasicComparison` and friends below.

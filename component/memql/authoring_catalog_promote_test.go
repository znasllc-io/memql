...
func TestPlanCatalogPromotion_MatchesDeterministicReuse(t *testing.T) {
	p, err := PlanCatalogPromotion("c1", "spec", "isAdmin", `spec actorEnvelope isAdmin = actor => actor.role == "admin"`, "b1")
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestPlanCatalogPromotion_RequiresIDAndBundle(t *testing.T) {
	if _, err := PlanCatalogPromotion("", "spec", "s", `spec actorEnvelope s = actor => actor.role == "admin"`, "b1"); err == nil {
		t.Errorf("expected an error for empty constructId")
	}
	if _, err := PlanCatalogPromotion("c1", "spec", "s", `spec actorEnvelope s = actor => actor.role == "admin"`, ""); err == nil {
...
spec actorEnvelope isAdmin = actor => actor.role == "admin"`, "b1")
	if err != nil {
		t.Fatalf("err: %v", err)
	}

...
spec actorEnvelope adminCheck = actor => actor.role == "admin"`)
	if err != nil {
		t.Fatalf("b: %v", err)
	}
	if a != b {
...
spec actorEnvelope s = actor => actor.role == "admin"`)
...
func TestCatalogMatchText_NoDescription(t *testing.T) {
	got, err := CatalogMatchText("spec", `spec actorEnvelope s = actor => actor.role == "admin"`)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
func TestDecideReuse_ExactMatch(t *testing.T) {
	key, _ := CatalogKey("spec", `spec actorEnvelope isAdmin = actor => actor.role == "admin"`)
	catalog := []CatalogEntry{{Name: "isAdmin", Kind: "spec", CatalogKey: key}}

	d, err := DecideReuse("spec", `spec actorEnvelope wantAdmin = actor => actor.role == "admin"`, catalog)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
...
spec actorEnvelope wantAdmin = actor => actor.role == "admin"`, catalog)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if d.ExactMatch != nil {
...
spec actorEnvelope wantOwner = actor => actor.role == "owner"`, catalog)
	if err != nil {
		t.Fatalf("err: %v", err)
	}
	if d.ExactMatch != nil {

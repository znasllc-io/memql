...
// TestParseViaLangparser_Equivalence was the #248 acceptance test
// asserting cross-parser AST equivalence. memql#328 deleted the
// recursive-descent memql parser (sole runtime parser is now the
// langparser via parseViaLangparser), so the equivalence test no
// longer has two parsers to compare. TestParseViaLangparser_CoversCorpus
// above is the surviving canary for runtime-query parsing.

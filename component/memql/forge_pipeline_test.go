...
//     role picks the next pipeline status on submission. Since #2235 the
//     routeRequest logic is a PURE decision and the role-guarded advanceRequest
//     writes live in the automation's per-role persist steps.
...
//     is derived from the new status. Since #2235 the recordTransition logic is
//     a PURE decision and the guarded recordRequestEvent writes live in the
//     automation's per-status persist steps.
...
func TestForgeRoutingContractMatchesDSL(t *testing.T) {
	...
	for role, target := range forgeRoutingContract {
		// The role guard now reads off the decide step's projection:
		// `field(step("decide"), "submitterRole") == "<role>"`.
...
func TestForgeEventKindContractMatchesDSL(t *testing.T) {
	...
	for status, kind := range forgeEventKindContract {
		// The per-status guard now reads off the decide step's projection:
		// `field(step("decide"), "toStatus") == "<status>"`.

...
func (s *engineActivationStore) LoadBundle(ctx context.Context, bundleId string) (AuthoringBundleRow, error) {
	q := fmt.Sprintf(`query authoringBundleById(bundleId:%s)`, langparser.QuoteString(bundleId))
...
func (s *engineActivationStore) SetBundleActive(ctx context.Context, bundleId string) error {
	_, err := s.engine.Execute(ctx, fmt.Sprintf(`mutation activateAuthoringBundle(bundleId:%s)`, langparser.QuoteString(bundleId)))
	return err
}

func (s *engineActivationStore) SetBundleRetired(ctx context.Context, bundleId string) error {
	_, err := s.engine.Execute(ctx, fmt.Sprintf(`mutation retireAuthoringBundle(bundleId:%s)`, langparser.QuoteString(bundleId)))
	return err
}

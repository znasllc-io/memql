...
async function browser({ stored=0, blocked=false, url='https://shop.example.test/cart?buyer=one#items' }={}) {
  ...
  const context=vm.createContext({window,document,location,history,URL,AbortController,
    Date:{now:()=>now},sessionStorage:{getItem(){if(blocked)throw Error('blocked');return stored},setItem(k,v){if(blocked)throw Error('blocked');stored=Number(v)}},
    setTimeout:(fn,ms)=>timer(fn,ms),clearTimeout:id=>timers.delete(id),setInterval:(fn,ms)=>timer(fn,ms,ms),
    fetch:async (url,options)=>{requests++;assert.equal(url,'/runtime-config.json');assert.equal(options.method,'HEAD');assert.equal(options.cache,'no-store');if(fail)throw Error('offline');return {ok:true,headers:{get:()=>latest}}}
  });
  const settle=async()=>{for(let i=0;i<8;i++)await Promise.resolve()};
  vm.runInContext(source,context);await settle();
  async function tick(ms){const end=now+ms;for(;;){let entry=[...timers].filter(([,t])=>t.at<=end).sort((a,b)=>a[1].at-b[1].at)[0];if(!entry)break;const [key,t]=entry;now=t.at;timers.delete(key);if(t.repeat)timers.set(key,{...t,at:now+t.repeat});t.fn();await settle()}now=end;await settle()}
  return {document,history,location,reloads,tick,get requests(){return requests},set latest(v){latest=v},set fail(v){fail=v},emit:async(name,event={})=>{for(const fn of events[name]??[])fn(event);await settle()},rerun:()=>vm.runInContext(source,context)};
}
...
test('cooldown survives page loads and blocked browser storage',async()=>{for(const blocked of [false,true]){const b=await browser({blocked,url:'https://shop.example.test/?__memql_reload=99000'});b.latest=B;await b.tick(32500);assert.equal(b.reloads.length,0);await b.tick(30000);assert.equal(b.reloads.length,1)}});

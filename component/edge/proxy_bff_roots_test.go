...
// and export routes are on the bff at ITS OWN root ("/artifacts"), routed by
// the front door on api.<domain>. A browser at portal.<domain> or os.<domain>
// POSTing to a bare "/artifacts" therefore does not reach the bff at all --
// the path resolves to no file in the bundle and takes the SPA fallback, so
// the upload is answered with index.html and a 200. That is a silent success
// for a request that stored nothing, which is why a hosted surface addresses
// these routes through the "/_memql" marker instead.
//
...
// not serve -- so upstreamPath strips the marker outright for that prefix.
// Both halves are asserted: that the own-root routes lose the marker, and that
// nothing else does.
...
func TestBffRootRoutesStillRequireTheAPIOptIn(t *testing.T) {
	...
	for _, path := range []string{"/_memql/artifacts", "/_memql/artifacts/a-1/content"} {

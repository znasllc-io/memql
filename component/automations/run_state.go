...
//
// Evaluator holds what a run has bound: the triggering event and every other
// seeded root, the names its statements bound (statement_scope.go), and the
...
// precondition, an argument, a `for` source -- is parsed once at load
// (PrepareExpressions) and evaluated by memql.EvalExpr over RunScope, which
// reads this state (run_scope.go).
...
type Evaluator struct {
	// custom holds the seeded roots (event, args, actor, config, partition)
	// and the run clock (now).
	custom map[string]any

	// variableResolver resolves var("NAME") to
	// v1:platform:partitionVariable, falling back to
	...
	// names is the name frame of a run's statements (statement_scope.go,
	// epic memql#5370), nil before the first statement. A Clone shares it; a
	// loop iteration or a parallel branch opens a child frame of its own
	// (childFrame).
	names *nameFrame
}
...
// Clone creates a copy of the run state for a nested context (a loop
// iteration, a parallel branch).
//
// Every resolver travels with the copy, the canonicalId one included. It used
// to be left behind, so a canonicalId() inside a loop or a parallel branch
// silently fell back to the raw value while the same call one level up
// canonicalised it (memql#5367).

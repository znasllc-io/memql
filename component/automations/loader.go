...
func (l *Loader) LoadAll() ([]*Automation, error) {
	...
	// Entry-point-logic registration invariant (memql#1707). A logic marked
	// `@entrypoint` is meant to be invoked directly via run_automation, but the
	// top-level dispatch only runs automations -- a bare logic is not a runnable
	// entry point. Auto-generate a wrapping automation for every @entrypoint
	// logic that no authored automation already wraps, so the class is fixed
	// structurally rather than per-logic.
	automations = l.appendEntryPointWrappers(automations)
...
// appendEntryPointWrappers compiles and appends a synthetic wrapping automation
// for every @entrypoint logic that the already-loaded `automations` set does
// not already wrap. The wrapper source is generated by the single
// memql.EntryPointLogicWrappers generator (shared with the dry-run source
// fetch), then compiled through the same compileMemQL pipeline as authored
// automations so the live + dry-run run_automation paths behave identically.
func (l *Loader) appendEntryPointWrappers(automations []*Automation) []*Automation {
	wrappers := memql.EntryPointLogicWrappers(l.logger)
	if len(wrappers) == 0 {
		return automations
	}

	// Set of logic constructs already wrapped by an authored automation, in
	// full `logicXxx` form. An @entrypoint logic in this set needs no synthetic
	// wrapper -- the authored automation already makes it runnable.
	wrapped := make(map[string]bool)
	for _, a := range automations {
		for _, invoked := range invokedLogicNames(a) {
			wrapped[invoked] = true
		}
	}

	for _, w := range wrappers {
		if wrapped[w.LogicName] {
			continue
		}
		origin := "entrypoint-wrapper:" + w.LogicName
		auto, err := l.compileMemQL(w.Source, origin)
		if err != nil {
			if l.logger != nil {
				l.logger.Warn("entry-point wrapper: compile failed",
					"component", ComponentName,
					"logic", w.LogicName,
					"error", err)
			}
...
// ValidateEntryPointLogics enforces the entry-point-logic registration
// invariant (memql#1707): every logic construct marked `@entrypoint` MUST have
// an invocation path -- an automation (authored or auto-generated) whose step
// invokes it -- so run_automation can reach it. Returns a descriptive error
// naming every orphan when the invariant is violated. Wired into CI so the
// build fails on any orphan entry-point logic.
//
// In normal operation LoadAll auto-generates a wrapper for every @entrypoint
// logic, so this returns nil; the audit is the backstop that proves the
// generation actually closed the gap (and catches a logic the generator could
// not wrap).
func (l *Loader) ValidateEntryPointLogics() error {
	wrappers := memql.EntryPointLogicWrappers(l.logger)
	if len(wrappers) == 0 {
		return nil
	}
	...
	if err != nil {
		return fmt.Errorf("entry-point invariant: loading automations: %w", err)
	}
	return validateEntryPointLogics(wrappers, automations)
}

// validateEntryPointLogics is the pure core of the audit: given the set of
// @entrypoint logics and the loaded automations, it reports any entry-point
// logic with no invoking automation. Split out so it is unit-testable against a
// hand-built orphan case without touching the DSL tree.
func validateEntryPointLogics(wrappers []memql.EntryPointLogicWrapper, automations []*Automation) error {
	invoked := make(map[string]bool)
	for _, a := range automations {
		for _, name := range invokedLogicNames(a) {
			invoked[name] = true
		}
	}
	var orphans []string
	for _, w := range wrappers {
		if !invoked[w.LogicName] {
			orphans = append(orphans, w.LogicName)
		}
	}
	if len(orphans) == 0 {
		return nil
	...
	return fmt.Errorf(
		"entry-point logic registration invariant violated: %d @entrypoint logic(s) have no wrapping automation and are uninvokable via run_automation: %s",
